...
import { agentTools, OperationContributionRegistry } from "@aicontainer/ragents";
...
export const runtimeFor = (setup: ReturnType<typeof setupRun>, directory: string, operations = new OperationContributionRegistry(),
  ...
  const processContextFor = async (runId: string) => ({runId, cwd: directory, root: directory, home: directory,
    env: {PATH: process.env.PATH, HOME: directory, RAGENTS_RUN_ID: runId, NO_COLOR: "1"}});
  setup.services.nativeTypeScriptExecutor = new NodeTypeScriptExecutor({directoryFor: () => path.join(directory, "native-programs"), processContextFor});
  const runtime = new ActorProgramRuntime({
    ...
    askService: () => askService, reservedToolNames: () => [], processContextFor,
