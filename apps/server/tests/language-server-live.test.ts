...
import { LanguageServerHost } from "../src/plugin-support/language-server/host.ts";
import { runManagedProcess } from "../src/plugin-support/managed-process.ts";
import { sanitizedEnv } from "../src/plugin-support/sandbox-tools.ts";
import type { SandboxProcessContext, SandboxServices } from "../src/plugin-support/workspace-sandbox-host.ts";
import { fsharpAdapter } from "../../../plugins/ragents.lsp-fsharp/server/index.ts";
import { roslynAdapter } from "../../../plugins/ragents.lsp-roslyn/server/index.ts";
import { typescriptAdapter } from "../../../plugins/ragents.lsp-typescript/server/index.ts";

const enabled = process.env.RAGENTS_LSP_TESTS === "1";
const skip = enabled ? false : "RAGENTS_LSP_TESTS=1 setzen (plus ROSLYN_LANGUAGE_SERVER / FSHARP_LANGUAGE_SERVER)";
...
const workspace = async (fixture: string): Promise<Workspace> => {
  ...
  const context: SandboxProcessContext = {
    runId: RUN,
    cwd: root,
    root,
    home,
  ...
  return {
    root,
    context,
    sandbox: { processContextFor: () => Promise.resolve(context), registerEditAnnotator: () => undefined, registerWorkspaceRoot: () => undefined, shutdown: async () => undefined },
    dispose: () => rm(directory, { recursive: true, force: true }),
  };
};

const restore = async (context: SandboxProcessContext, solution: string): Promise<void> => {
  let output = "";
...
test("Roslyn reports C# errors of an edited file without a build", { skip, timeout: 600_000 }, async () => {
  ...
  const host = new LanguageServerHost(roslynAdapter, ws.sandbox);
  try {
...
test("FSAC reports F# errors of an edited project file", { skip, timeout: 600_000 }, async () => {
  const ws = await workspace("dotnet");
  const host = new LanguageServerHost(fsharpAdapter, ws.sandbox);
  try {
    await restore(ws.context, "Sample.sln");
...
test("TypeScript reports errors of an edited file and lists changed files via git", { skip, timeout: 300_000 }, async () => {
  ...
  const host = new LanguageServerHost(typescriptAdapter, ws.sandbox);
  try {
    ...
    assert.equal(await host.diagnostics(RUN, undefined, false), "Keine geänderten TypeScript-Dateien im Arbeitsverzeichnis");
