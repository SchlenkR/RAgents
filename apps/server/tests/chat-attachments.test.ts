...
import { coreSources, openRunAccess, startRpcServer } from "./rpc-fixture.ts";
...
test("TypeScript actors reject chat before publishing attachments or inputs while program inputs remain available", async () => {
  ...
  try {
    ...
    for (const primary of [false, true]) {
      ...
      assert.deepEqual(await data.session.capabilities(script.id, null), { input: [], model: "TypeScript" });
      if (primary) assert.deepEqual(await data.session.capabilities("primary", null), { input: [], model: "TypeScript" });
    }
...
test("chat methods await enqueue, attachments are delivered as files and oversized requests are refused", async (t) => {
  ...
  const server = await startRpcServer(t, {
    ...
    routes: [attachmentContentRoute(provider, openRunAccess)],
