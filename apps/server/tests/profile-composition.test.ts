...
import { compositionEnvironment, coreFixture, minimalFixture, type CompositionFixture } from "./fixtures/profile-composition/profiles.ts";
...
test("die neutrale Core-Fixture komponiert echte Plugins und Ordnerbeiträge vollständig", async () => {
  const host = await composed(coreFixture);
  const profile = host.publicProfile();

  assert.deepEqual(profile.product, coreFixture.product);
  assert.deepEqual(profile.plugins.map((entry) => entry.id), [...coreFixture.plugins]);
  ...
  assert.deepEqual(host.tools.describe().map((tool) => tool.name).sort(), [
    "ask_user", "bash", "canvas_layout_replace", "edit",
    "fsharp_diagnostics", "fsharp_open",
    ...
    "quick_answer", "read", "roslyn_diagnostics", "roslyn_open", "run_stop",
    "show_document", "todo_replace",
    "typescript_api", "typescript_eval", "typescript_diagnostics", "typescript_open", "watch_create", "watch_list", "watch_remove", "write",
  ].sort());
...
test("eine reduzierte Fixture entfernt optionale Actor-Programm- und Referenzbeiträge", async () => {
  ...
  assert.deepEqual(host.tools.describe().filter((entry) => entry.nativeTool).map((entry) => entry.name).sort(), ["bash", "edit", "read", "typescript_api", "typescript_eval", "write"]);
...
test("jedes Plugin trägt sein requires aus dem Modulvertrag ins Manifest", async () => {
  ...
  assert.deepEqual(manifests.get("ragents.actor-programs"), ["ragents.ask"]);
...
test("function catalogs keep compact descriptions while runtime functions retain detailed instructions", async () => {
  const { createReadToolDefinition, createEditToolDefinition, createWriteToolDefinition, createBashToolDefinition } = await import("@aicontainer/agent");
...
test("kein Werkzeug bietet dem Modell eine Union an der Wurzel seines Eingabeschemas", async () => {
  const { agentTools } = await import("@aicontainer/ragents");
  ...
  for (const fixture of [coreFixture, minimalFixture]) {
