...
test("explicit non-default startup selections are persisted atomically with run creation", async () => {
  ...
  const fixture = createFixture(runId);
  fixture.session.selectStartOption(systemPromptStartOptionId, { promptIds: ["special"], shareWithAgents: true });
  fixture.session.selectStartOption(modelStartOptionId, { model: "other-first", thinking: "high" });
