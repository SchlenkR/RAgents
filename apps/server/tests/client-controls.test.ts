...
import { browserRuntimeStyles } from "../../../plugins/ragents.actor-programs/server/client-runtime.ts";
...
test("normal control modules render with the shared React and preserve native chat styling", async () => {
  ...
  const bundle = buildSync({
    stdin: { contents: `import * as UI from ${JSON.stringify(fileURLToPath(new URL("../../../plugins/ragents.actor-programs/client-ui/index.tsx", import.meta.url)))}; export { UI };`, resolveDir: process.cwd(), loader: "tsx" },
  ...
  const question = render(`{ messages: [{ key: "question", role: "question", text: "Welche Farbe?",
    question: { callId: "question-1", options: ["Blau", "Rot"] } }] }`);
  assert.match(question, /Welche Farbe\?/);
  assert.match(question, /<li>Blau<\/li>/);
  assert.match(question, /<li>Rot<\/li>/);
  assert.doesNotMatch(question, /<input|<button|<textarea/);
  const answerableQuestion = renderControl("ChatMessages", `{
    messages: [{ key: "question", role: "question", text: "Welche Farbe?",
      question: { callId: "question-1", options: ["Blau", "Rot"] } }],
    onAnswerQuestion: () => {},
  }`);
  assert.match(answerableQuestion, /<button/);
  assert.match(answerableQuestion, /<input/);
