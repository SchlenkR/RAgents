...
const fixture = async (t: TestContext) => {
  ...
  const runtime = await ModelRuntime.create({ authPath: path.join(directory, "auth.json"), modelsPath: null });
...
test("corrupt persisted title settings and unavailable provider catalogs fail immediately", async (t) => {
  ...
  await assert.rejects(TitleSettingsStore.create(path.join(directory, "new.json"), runtime, initial, "unknown-provider"), /fehlen Textmodelle ohne Reasoning/);
