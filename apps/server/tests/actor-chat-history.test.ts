...
test("a failed turn closes its open tool calls with its reason in the primary and the actor chat", () => {
  const f = fixture();
  try {
    const actor = f.coordinator.id;
...
test("stopping an actor mid-turn names the reason once in the primary and the actor chat, separate commands both", () => {
  const f = fixture();
  try {
    const actor = f.coordinator.id;
...
test("shared action payloads preserve actor routing, primary asker labels and dismissal results", () => {
  const f = fixture();
  try {
...
test("delivered inputs retain senders and attachments while actions and stops stay with their actor", () => {
  const f = fixture();
  try {
    const a = f.coordinator.id, b = f.reviewer.id;
