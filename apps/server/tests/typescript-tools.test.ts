...
import { claimTurn, defineRunFunction, ToolRegistry, TurnToolset, type JsonValue } from "@aicontainer/ragents";
...
const fixture = async (t: TestContext, toolNames: string[] | null = null) => {
    ...
    }).register(createTypeScriptToolContributor({
        processContextFor: async (runId) => ({
            runId, cwd: workspace, root: workspace, home: workspace, env: process.env,
            additionalRoots: [actors], workspaceAliases: { "@actors": actors },
        }),
    }));
...
test("snippet files resolve workspace aliases and reject escaped paths and conflicting sources", async (t) => {
    ...
    await assert.rejects(run.call("eval-outside", "typescript_eval", { path: "../outside.ts" }), /außerhalb des Arbeitsverzeichnisses/);
...
test("inline compile failures keep a source snapshot while unreadable paths fail without inventing source", async (t) => {
    ...
    await assert.rejects(run.call("path-missing", "typescript_eval", { path: "@actors/missing.ts" }), /ENOENT/);
    const events = run.setup.runtime.events(run.setup.view.id);
    const snapshots = events.filter((event) => event.type === "tool.call.source");
