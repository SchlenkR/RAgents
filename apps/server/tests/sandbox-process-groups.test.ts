...
import { createSandboxTools, processGroupExists, workspaceProcessContext } from "@ragents/workspace-executor";
...
const fixture = async (t: TestContext) => {
  ...
  const sandbox = await createSandboxTools("process-group-test", async () => workspaceProcessContext({
    runId: "process-group-test", cwd: directory, root: directory, home: { home: directory }, logDirectory: directory, hostRoot: undefined,
  }));
  t.after(async () => {
    t.mock.restoreAll();
  ...
  const run = sandbox.tools.get("bash")!;
  const bash = {
    execute: (toolCallId: string, input: { command: string; timeout?: number }, signal?: AbortSignal,
      onUpdate?: (result: { content?: ReadonlyArray<{ type: string; text?: string }> }) => void) =>
      run(input, { toolCallId, ...(signal ? { signal } : {}), ...(onUpdate ? { onUpdate } : {}) }) as Promise<{ content: { text?: string }[] }>,
  };
  return { bash, sandbox };
};
