...
test("coordinator prompts survive primary handover while dynamic guides remain explicit", async (t) => {
  ...
  plugins.register({ manifest: { id: "test.prompt" }, register: (host) => {
    ...
    host.prompts(
      ...
      boundToTools({
        ...
        order: 4,
        delivery: "initial",
        render: () => "Executor of the server.",
        renderForRun: (id) => `Executor of ${id}.`,
      }, "build_demo"),
    );
  } });
  ...
  try {
    ...
    assert.match(engine.systemPromptFor(runId), /Executor of prompt-context\./);
    assert.doesNotMatch(engine.systemPromptFor(runId), /Executor of the server\./);
    engine.start();
    ...
    assert.match(first.systemPrompt, /Executor of prompt-context\./);
    ...
    assert.match(worker.systemPrompt, /Executor of prompt-context\./);
    ...
    for (const handle of ["worker", "primary"]) {
      ...
      assert.match(request.systemPrompt, new RegExp(`Executor of ${fork.id}\\.`));
    }
  } finally {
