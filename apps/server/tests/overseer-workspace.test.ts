...
const isolated = async () => {
  ...
  const workspace: SessionWorkspace = {
    cwd,
    hostSandbox: { home: cwd, readOnlyRoots: [{ directory: journals, environmentVariable: "RAGENTS_JOURNAL_DIR" }] },
...
test("global shell reaches authenticated HTTP without credentials in ordinary run environments", async () => {
  ...
  try {
    ...
    const normal = sandboxEnvironment({ ACCESS_TOKEN: token, RAGENTS_API_TOKEN: token, RAGENTS_JOURNAL_DIR: f.journals },
      { home: { home: f.cwd }, additions: sandboxRunEnvironment("normal") });
