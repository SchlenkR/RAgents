...
test("http: requests are answered with JSON-RPC results, failures and rights checks", async (t) => {
  ...
  assert.deepEqual((await f.post({ jsonrpc: "2.0", id: 1, method: "test.greet", params: { name: "Ronald" } })).body, { jsonrpc: "2.0", id: 1, result: { text: "Hallo Ronald" } });
  ...
  const invalid = (await f.post({ jsonrpc: "2.0", id: 3, method: "test.greet", params: { name: 5 } })).body as { error: { code: number } };
  ...
  const contract = (await f.post({ jsonrpc: "2.0", id: 7, method: "test.broken", params: {} })).body as { error: { code: number } };
