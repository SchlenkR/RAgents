...
test("the run function finds an actor by handle regardless of case and Unicode composition", async (t) => {
  const f = fixture(t);
  const tool = createTranscriptTool(() => f.runtime);
  const scope = { caller: { runId: f.runId, actorId: f.agent.id } } as never;
  const byHandle = await tool.run(scope, "call-case", { actor: "@WORKER" }) as { actorId: string };
  assert.equal(byHandle.actorId, f.agent.id);
});
