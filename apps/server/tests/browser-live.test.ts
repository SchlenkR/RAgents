...
test("Echter Browser bedient Formular und Iframe, isoliert Cookies und prüft Screenshots und Abbruch", { skip: process.env.RAGENTS_BROWSER_TESTS !== "1", timeout: 60_000 }, async (context) => {
  ...
  const browser = new RunBrowser({
    filesFor: async (runId) => path.join(directory, runId),
  ...
  const pending = browser.click("one", { role: "button", name: "Wird nie angezeigt" }, abort.signal);
  ...
  const restored = new RunBrowser({ filesFor: async (runId) => path.join(directory, runId) });
