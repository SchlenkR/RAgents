...
import type { ResolvedWorkspaceRoot } from "../src/plugin-support/workspace-paths.ts";
...
test("registered private roots provide file aliases and Bash variables without exposing another run", async () => {
  ...
  const tools = await createSandboxTools("run-1", {
    cwd: root, currentRoot: async () => root, ensureWritable: async () => root, runOperation: (operation) => operation(),
  }, [], undefined, { home: path.join(directory, "home") }, undefined, undefined, async () => roots);
  ...
  } finally { await tools.shutdown(); await rm(directory, { recursive: true, force: true }); }
});
...
test("the existing language server resolves the run's app alias before launch and rejects foreign roots", async () => {
  ...
  const sandbox = new WorkspaceSandboxHost({
    ...
    homeFor: async () => ({ home: directory }), filesFor: async () => undefined,
  });
  ...
  try {
    const context = await sandbox.processContextFor("run-1");
