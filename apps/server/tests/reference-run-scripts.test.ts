...
import { ACTOR_PROGRAMS_STATE_ID, resolveActorView, type ActorProgramState } from "../../../plugins/ragents.actor-programs/contract.ts";
...
test("the balcony setup builds its advisor, view and canvas through native core tools before any model input", async () => {
  ...
  const session = new RunChatSession({
    ...
    scriptEntryFor: () => undefined, actorPrograms: host.service(actorProgramsToken),
  });
  try {
    ...
    assert.ok(layout.nodes.some((node) => "entity" in node && node.entity === `app:${app.view.id}`));
  } finally {
    session.dispose();
...
for (const sample of ["word-game", "learning-afternoon"]) {
  test(`${sample} starts from its real view function and completes through native actors and subscriptions`, async () => {
    ...
    const session = new RunChatSession({
      ...
      prompt: () => "Unexpected coordinator", assertUsable: () => {}, prepare: async () => {}, prepareWorkspace: async () => {},
      scriptEntryFor: () => undefined, actorPrograms: programs,
    });
    try {
      const { script, ...descriptor } = entry;
      await session.startPackageAndWait({ ...script, entry: { ...descriptor, coordinator: script.coordinator } }, null);
