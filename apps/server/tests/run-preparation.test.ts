...
const runtimeFixture = (t: TestContext, complete: (...args: Parameters<ModelRuntime["completeSimple"]>) => Promise<AssistantMessage> = async () => fauxAssistantMessage("Welche Kennzahlen soll die Analyse zeigen?")) => {
  ...
  const runtime = { getModel: (provider: string, id: string) => provider === model.provider && id === model.id ? model : undefined,
    hasConfiguredAuth: () => true, isUsingOAuth: () => false, streamSimple } as unknown as ModelRuntime;
  return { runtime, streamSimple, options: { runtime, prompt: preparationPrompt, agentDir: directory,
...
const sessionFixture = () => {
  ...
  const engine = { journal, runtime, live: new LiveBus(), scheduler: { isRunning: () => false }, catalog: new StaticModelCatalog(models, profiles),
    catalogModels: models, startOptions } as unknown as Engine;
  ...
  const session = new RunChatSession({ engine, id: "draft", coordinator: { handle: "coordinator", displayName: "Koordinator", profile: "coordinator",
    ...
    prepare: unused, prepareWorkspace: unused, scriptEntryFor: () => undefined, actorPrograms: unavailableActorPrograms });
...
test("preparation uses the same coordinator model selection before start and creates no journal or workspace", async (t) => {
  ...
  assert.deepEqual(session.preparationSelection(), { provider: "test", model: "default", thinking: "low" });
  session.selectStartOption("ragents.model", { model: "selected", thinking: "high" });
  const { options, streamSimple } = runtimeFixture(t);
  const result = await prepareRunMessage({ ...options, selection: session.preparationSelection() });
  ...
  session.dispose();
  assert.throws(() => session.preparationSelection(), isDomain(409));
});
...
test("provider rejects concurrent preparation and a response after a run begins", async (t) => {
  ...
  const pending = fixture.provider.prepareRunMessage("draft", request(), new AbortController().signal);
  await fixture.started.promise;
  await assert.rejects(fixture.provider.prepareRunMessage("draft", request(), new AbortController().signal), isDomain(409, "preparation-busy"));
...
test("delete aborts preparation as soon as deletion is requested", async (t) => {
  ...
  await assert.rejects(pending, isDomain(499));
  await deleting;
  await assert.rejects(fixture.provider.prepareRunMessage("draft", request(), new AbortController().signal), isDomain(409, "run-deleting"));
});
...
test("provider lists the current journal revision and leaves preparing sessions without one", async (t) => {
  ...
  Object.assign(provider, {
    ...
    plugins: {
      ...
      sessionMetadata: { describe: async () => ({}) },
    },
  });
