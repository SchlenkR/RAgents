...
import { allowedWorkspacePath, expandWorkspaceAlias } from "../plugin-support/workspace-paths.js";
import { sandboxServicesToken, type SandboxProcessContext } from "../plugin-support/workspace-sandbox-host.js";
...
export interface TypeScriptToolOptions {
    readonly processContextFor: (runId: string) => Promise<SandboxProcessContext>;
}
...
const sourceOf = async (input: { code?: string; path?: string }, context: SandboxProcessContext): Promise<string> => {
    if (input.code !== undefined) return input.code;
    const requested = expandWorkspaceAlias(input.path!, context.workspaceAliases ?? {});
    const filename = await allowedWorkspacePath(path.resolve(context.cwd, requested), [context.root, context.cwd, ...context.additionalRoots ?? []]);
    ...
    const tools: readonly RunFunction[] = [
        ...
        defineRunFunction({
            ...
            run: async (scope, id, input) => {
                ...
                const processContext = await options.processContextFor(scope.caller.runId);
                const code = await sourceOf(input, processContext);
...
const registeredHosts = new WeakSet<PluginHost>();

export const registerTypeScriptFunctions = (host: PluginHost): void => {
    if (registeredHosts.has(host)) return;
