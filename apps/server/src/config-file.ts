...
const SECRET_KEY_PATTERN = /(_PAT|_KEY|_TOKEN|_SECRET|_PASSWORD)$/;
...
const resolveIdentity = (location: string, entry: Record<string, unknown>): AccessUser => {
  ...
  if (entry.startEntries !== undefined && (!Array.isArray(entry.startEntries)
    || !entry.startEntries.every((id) => typeof id === "string" && /^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/.test(id))
...
export const resolveProfileUsers = (
  ...
  return raw.map((value: unknown, index: number) => {
    ...
    if (Object.keys(entry).some((key) => !["id", "label", "password", "rights", "startEntries"].includes(key))) {
      throw new Error(`${location} erlaubt nur id, label, password, rights und startEntries`);
    }
    ...
    return Object.freeze({ ...user, password });
  });
};
...
const materialize = (value: Exclude<ConfigValue, EnvironmentReference>): string => {
...
const entryFor = (source: string, section: string, key: string, value: ConfigValue): ConfigFileEntry => {
  ...
  if (isEnvironmentReference(value)) {
    ...
    if (resolved === undefined) {
      throw new Error(
        ...
        + "trage sie in die ~/.zshrc ein; Secrets stehen nie in der Konfigurationsdatei.");
    }
...
const parseEntries = (source: string, raw: RAgentsConfig): readonly ConfigFileEntry[] => {
  ...
  for (const [section, sectionValue] of sections) {
    for (const [key, value] of Object.entries(sectionValue)) {
      entries.push(entryFor(source, section, key, value));
    }
  }
...
const readProfileConfiguration = async (root: string): Promise<LoadedConfigFile | undefined> => {
  ...
  return { path: source, entries: parseEntries(source, raw), users, anonymousUser };
};
