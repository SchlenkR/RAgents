...
import type { NativeTypeScriptBinding, NativeTypeScriptExecutor, NativeTypeScriptRequest, NativeTypeScriptResult } from "@aicontainer/ragents";
import type { JsonValue } from "@aicontainer/ragents";
import { startManagedService, type ManagedService } from "./managed-process.js";
import type { SandboxProcessContext } from "./workspace-sandbox-host.js";
...
export class NodeTypeScriptExecutor implements NativeTypeScriptExecutor {
  ...
  async #start(request: NativeTypeScriptRequest, instanceId: string, fingerprint: string, controller: AbortController, callerSignal?: AbortSignal): Promise<Backend> {
    ...
    const processContext = await this.#options.processContextFor(request.context.runId);
