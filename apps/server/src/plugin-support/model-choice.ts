...
import { getBuiltinModels, type BuiltinProvider } from "@aicontainer/ai/providers/all";
...
export const modelThinkingOptions = (provider: string, model: string): readonly ThinkingLevel[] => {
  const metadata = getBuiltinModels(provider as BuiltinProvider).find((entry) => entry.id === model);
...
export const modelChoiceFromEnvironment = (
  ...
  const options = (): readonly string[] => {
    ...
    const list = configured.length > 0 ? configured : defaults.fallback();
    const duplicate = list.find((model, index) => list.indexOf(model) !== index);
  ...
  const reasoningMap = (): ReadonlyMap<string, readonly ThinkingLevel[]> => {
    ...
    for (const entry of env.list("AGENT_MODEL_REASONING")) {
      ...
      const supported = modelThinkingOptions(defaults.provider, model);
  ...
  return Object.freeze({
    ...
    thinkingOptionsFor: (model: string | null) => {
      ...
      return reasoningMap().get(selected) ?? modelThinkingOptions(defaults.provider, selected);
    },
  });
};
