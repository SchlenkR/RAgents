...
const hostDirectory = fileURLToPath(new URL("../../../../web/src/", import.meta.url));
...
export const buildTailwind = async (sources: readonly TailwindSource[], entry = frameStylesheet): Promise<string> => {
  ...
  const scanner = new Scanner({ sources: [
    ...compiler.sources,
    ...[hostDirectory, ...sources].map((source) => typeof source === "string" ? { base: source, pattern: "**/*", negated: false } : { ...source, negated: false }),
  ] });
