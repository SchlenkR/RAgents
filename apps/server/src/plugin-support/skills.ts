...
import { parseFrontmatter } from "@aicontainer/agent";
import type { StartEntryContribution } from "@aicontainer/ragents";
...
const skillStartFrom = (directory: string, idPrefix: string): SkillStartEntry | undefined => {
  ...
  flag(file, "disable-model-invocation", fields.get("disable-model-invocation"));
  ...
  if (!flag(file, "start", fields.get("start"))) return undefined;
...
export const skillsFromDirectory = (directory: string, idPrefix: string): FolderSkills => {
  ...
  const startEntries = paths.flatMap((skillPath) => {
    const entry = skillStartFrom(skillPath, idPrefix);
    return entry ? [entry] : [];
  });
