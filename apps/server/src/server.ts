...
// scripts/start.sh --dev: das Web kommt vom Vite-Dev-Server, die Bundles hält ein Watch aktuell.
...
const accessGate = async (
  ...
  // Das gebaute Web, die ausgelieferten Dateien der Web-Hälften und das Stylesheet sind frei, jede Datenroute verlangt den Token; so laden iframes ohne Cookie ihre Skripte.
...
const rebuild = (command: string): string => checkout ? `im Checkout mit ${command} neu bauen` : "das Paket ist unvollständig; neu installieren";
if (!devMode) {
  ...
  if (stale.length > 0) throw new Error(`Eingebaute Bundles passen nicht mehr zu ihren Quellen unter plugins/: ${stale.join(", ")}; ${rebuild("pnpm build:plugins")}`);
}
...
/** Bindet den Port und nennt den tatsächlich gebundenen; ein belegter Port bricht den Start ab. */
const listenOn = (target: Server, port: number, host: string | undefined): Promise<number> => new Promise((resolve, reject) => {
  ...
  target.listen(port, host, () => {
    ...
    if (!address || typeof address === "string") reject(new Error(`Der Server lauscht nicht auf einem TCP-Port: ${String(address)}`));
    else resolve(address.port);
  });
});

// Der Port steht vor dem Aufbau fest, damit jede Adresse ab dem Start stimmt; Anfragen warten, bis der Server fertig ist.
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (url.pathname === STYLESHEET_PATH) {
    // Ein Bundle kann gerade neu entstehen; das kostet diesen Abruf, nicht den Server.
...
// Ein Aufrufer wie die VS-Code-Erweiterung nennt sich in RAGENTS_PARENT_PID; verschwindet er, endet der Host wie bei SIGTERM.
watchParentProcess({
  parentPid: process.env.RAGENTS_PARENT_PID,
  onGone: (pid) => {
    console.error(`Der aufrufende Prozess ${pid} lebt nicht mehr; der Host fährt herunter.`);
    shutdown();
  },
});
