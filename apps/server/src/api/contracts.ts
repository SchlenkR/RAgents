...
import type { PublicPluginProfile } from "@aicontainer/ragents/src/plugin-types";
import { defineChannel, defineOperation } from "@aicontainer/ragents/src/rpc/contract";
import { openJson, runContracts } from "@aicontainer/ragents/src/http/contracts";
...
export const coreContracts = {
  ...
  chat: {
    send: defineOperation({
      ...
      description: "Eine Nachricht an den Koordinator des Runs; startet einen neuen Run oder funkt in einen laufenden. Rechte: runs.read und runs.write, für einen neuen Run runs.create; beim globalen Chat dessen Rechte.",
      input: Type.Object({
        runId,
    ...
    start: defineOperation({
      ...
      description: "Einen Run über einen Skill- oder Run-Script-Einstieg starten, ohne Nachricht. Rechte: runs.read, runs.write und die Freigabe des Einstiegs.",
    ...
    stop: defineOperation({
      ...
      description: "Den laufenden Turn des Koordinators abbrechen. Rechte: runs.read und runs.write.",
      input: Type.Object({ runId }, { additionalProperties: false }),
      result: Type.Null(),
    }),
