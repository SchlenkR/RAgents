...
import { ACCESS_TOKEN_QUERY, type AccessSnapshot, type AccessUser } from "@aicontainer/ragents";
...
export const createAccessSessionManager = (options: AccessSessionOptions) => {
  ...
  const snapshot = (request: IncomingMessage): AccessSnapshot => ({
    enabled,
    user: enabled ? sessionFor(request)?.user ?? null : options.anonymousUser ?? null,
  });
