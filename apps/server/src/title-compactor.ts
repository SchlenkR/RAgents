...
export const createTitleCompactor = (options: TitleCompactorOptions): TitleCompactor => {
  ...
  const modelRuntime = () => runtime ??= options.modelRuntime ?? ModelRuntime.create({
    authPath: path.join(options.agentHomeDir, "auth.json"),
    modelsPath: path.join(options.agentHomeDir, "models.json"),
  });
