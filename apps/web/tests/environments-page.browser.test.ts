...
test("die Seite Umgebungen legt im Dialog an, behält Eingaben bei einem Fehler, bearbeitet, meldet an und entfernt nach einer Rückfrage im Dialog",{skip:process.env.RAGENTS_BROWSER_TESTS!=="1",timeout:120_000},async(context)=>{
  const scratch="/private/tmp/ragents-environments-page";await mkdir(scratch,{recursive:true});const directory=await mkdtemp(`${scratch}/page-`);const root=fileURLToPath(new URL("../../../",import.meta.url));
...
function App(){const[state,setState]=useState(window.fixture.initial);window.fixture={...window.fixture,setState};window.fixture.sent??=[];return <PanelPage state={state} send={action=>window.fixture.sent.push(action)}/>};createRoot(document.getElementById('root')).render(<App/>);`,resolveDir:`${root}apps/web`,loader:"tsx"},jsx:"automatic",bundle:true,platform:"browser",format:"iife",outfile:`${directory}/app.js`,plugins:[tailwindPlugin([])],logLevel:"silent"});
  const initial={theme:"dark",page:"environments",targets:[],profileSuggestions:suggestions};
  ...
  try{
    ...
    assert.equal(await page.getByText('Noch keine Umgebung.').count(),1);
    ...
    await page.locator('#target-url').fill('localhost:4715');
    await page.locator('#target-name').fill('werkstatt');
    ...
    assert.equal(await page.locator('#target-url').inputValue(),'localhost:4715','die abgelehnte Eingabe bleibt stehen');
    assert.equal(await page.locator('#target-name').inputValue(),'werkstatt');
    await page.locator('#target-url').fill('http://localhost:4715');
    await page.getByRole('button',{name:'Anlegen'}).click();
    await patch(page,{problem:undefined,targets:[server('werkstatt',{kind:'connected'},{user:'alex',savedLogin:true})]});
    await closed();
    ...
    assert.equal(await page.locator('#target-name').inputValue(),'core','die gewählte Datei benennt das Profil');
    await page.getByRole('button',{name:'Anlegen'}).click();
    ...
    await patch(page,{targets:[server('werkstatt',{kind:'connected'},{user:'alex',savedLogin:true}),core]});
    await closed();
    ...
    await page.waitForFunction(()=>(document.getElementById('target-profile-file') as HTMLInputElement|null)?.value==='/anderswo/ragents.config.pruef.ts');
    assert.equal(await page.locator('#target-name').inputValue(),'pruef');
    await page.getByRole('button',{name:'Abbrechen'}).click();
    await closed();
    ...
    await dialog.waitFor();
    assert.equal(await page.locator('#target-url').inputValue(),'http://localhost:4715');
    await page.locator('#target-name').fill('werkstatt-neu');
    ...
    await patch(page,{targets:[server('werkstatt-neu',{kind:'login-required',mode:'password'},{loginUser:'alex',savedLogin:true}),core]});
    await closed();
    ...
    await patch(page,{targets:[server('werkstatt-neu',{kind:'connected'},{user:'alex',savedLogin:true}),core]});
    await closed();
    ...
    await patch(page,{targets:[server('werkstatt-neu',{kind:'connected'},{user:'alex',savedLogin:true})],missingSecrets:['SERVICE_TOKEN','SERVICE_URL']});
