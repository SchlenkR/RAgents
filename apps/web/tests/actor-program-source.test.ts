...
import { actorProgramSourceFrom } from "../../../plugins/ragents.actor-programs/web/api.ts";
import { ActorProgramsContext } from "../../../plugins/ragents.actor-programs/web/context";
import { ACTOR_PROGRAMS_STATE_ID } from "../../../plugins/ragents.actor-programs/contract";
...
const providerValue = { runId: view.id, api: { source: () => assert.fail("Source is loaded only after opening its tab") } } as unknown as NonNullable<ContextType<typeof ActorProgramsContext>>;
const withProgram = (children: ReactNode) => createElement(ActorProgramsContext.Provider, { value: providerValue }, children);
...
const value = { api, runId: initialView.id };
function Harness() {
  ...
  return createElement(AccessContext.Provider, { value: { ...access, logout: async () => {} } }, fixture.provider ? createElement(ActorProgramsContext.Provider, { value }, inspector) : inspector);
}
