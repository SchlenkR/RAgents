...
import type { RunAction, RunActor, RunActorInput, RunTurn, RunView } from "../../../plugins/ragents.orchestration/web/run-view.ts";
...
test("pending questions ask for an answer without a spinner even while a turn is active", () => {
  const question: RunAction = {
    id: "question-1", askedBy: "setup", kind: "question", question: { options: ["Weiter"], multi: false },
    title: "Weiterarbeiten?", description: null, parameters: {}, input: null, status: "pending",
    proposedAt: at, resolvedAt: null, resolvedBy: null, response: null,
  };
  const result = state({ running: true, view: view({ actors: [actor()], actions: [question] }) });
  assert.equal(result?.kind, "waiting");
  assert.match(result!.detail, /Rückfrage/);
