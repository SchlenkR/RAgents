...
test("chat targets resolve handles like the engine: any case and composition", () => {
  ...
  const reviewerNfd = { ...worker, handle: "prüfer" };
  const reviewer = session({ runView: { ...view, actors: [primary, reviewerNfd] } });
  assert.equal(resolveChatActor(reviewer, "@Prüfer").actor.id, worker.id);
});
