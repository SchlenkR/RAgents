...
export const fileSelection = (current: readonly File[], incoming: readonly File[], options: Pick<FilePickerProps, "accept" | "multiple" | "maxFiles" | "maxBytes">): File[] => {
  ...
  if (!Number.isInteger(maximum) || maximum < 1 || !Number.isFinite(bytes) || bytes < 1) throw new Error("File limits must be positive numbers.");
  if (!incoming.length) throw new Error("The selection contains no readable files.");
  if (options.multiple === false && incoming.length > 1) throw new Error("Select exactly one file.");
  ...
  if (result.length > maximum) throw new Error(`Select at most ${maximum} files.`);
  if (result.reduce((sum, file) => sum + file.size, 0) > bytes) throw new Error(`The selection exceeds ${bytes} bytes.`);
  return result;
};
