...
function prepare({ nodes, edges, direction = "right", height = 360, viewport = "interactive", detailLevel = "summary", layout = "layered" }: FlowDiagramProps) {
  ...
  const cards = nodes.map((node): CardNode => {
    ...
    for (const action of node.actions ?? []) {
      if (typeof action.id !== "string" || !action.id.trim() || actionIds.has(action.id)) throw new Error("Actions require unique, non-empty IDs.");
      actionIds.add(action.id);
      if (typeof action.label !== "string") throw new Error("Action labels must be text.");
    }
...
async function layeredLayout(cards: CardNode[], links: Link[], direction: "right" | "down"): Promise<Placement> {
  ...
  const edges: RouteEdge[] = links.map((edge) => {
    ...
    if (!route.sections?.length) throw new Error("Automatic routing is missing for an edge.");
...
export function FlowDiagram(props: FlowDiagramProps) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- inputKey encodes every prop the preparation reads
  }, [inputKey]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- the graph key identifies the prepared graph
  }, [key]);
