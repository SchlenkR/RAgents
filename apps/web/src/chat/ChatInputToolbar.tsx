...
import { Button, cn, Toggle, useFileInput } from "../ui";
import { KeyboardEvent, ReactNode, Ref, useCallback, useEffect, useImperativeHandle, useRef, useState } from "react";
...
export function ChatInputToolbar({
  ...
  const textareaClasses = cn(
    "resize-none border-0 bg-transparent text-[13px] leading-normal text-foreground placeholder:text-muted-foreground focus:outline-none",
  ...
  const sendButtons = <>
          {running && onStop && !hasContent && (
            <Button aria-label={alleTexte.stop} className="flex-none" onClick={onStop} size="icon-sm" title={alleTexte.stop} variant="secondary">
              <SquareIcon fill="currentColor" size={14} />
            </Button>
          )}
          {(!running || !onStop || hasContent) && (
            <Button
