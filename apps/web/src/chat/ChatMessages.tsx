...
import { DetailMode, Message, prettyJson, stepState, ToolInfo } from "./types";
import { ChatTexts, defaultTexts } from "./texts";
import { Markdown, MarkdownLinks, type LinkClickHandler } from "./Markdown";
import { StepPopover } from "./StepPopover";
import { QuestionCard } from "./QuestionCard";
import { WorkingScenes } from "./WorkingScenes";
import { attachmentDownloadUrl, formatAttachmentSize } from "./attachments";
import { CheckIcon, ChevronDownIcon, ChevronRightIcon, CopyIcon, LayersIcon, SparklesIcon, WrenchIcon, XIcon } from "lucide-react";

const stepClasses = "mt-4";
const denseStepClasses = "mt-2";
...
const bubbleBodyClasses = "relative max-w-[86%] px-3 py-2 leading-[1.625] [overflow-wrap:anywhere] @max-[620px]/chat:max-w-full";

// Die Aktionen liegen ueber der Nachricht und erscheinen erst beim Zeigen; ohne Zeiger bleiben sie sichtbar.
const messageActionsClasses = cn(
  "absolute top-1 right-1 z-[1] flex rounded-sm bg-secondary opacity-0",
...
export function ChatMessages({
  ...
  const [scroll] = useState(() => createChatScroll(setAmEnde));
  ...
  const pruefeEnde = useCallback(() => {
    ...
    if (bereich) scroll.scroll(bereich, scrollbarDragging.current);
  }, [scroll]);
  ...
  useLayoutEffect(() => {
    const bereich = scrollBereich.current;
    if (!bereich) return;
    const align = () => scroll.layout(bereich, scrollbarDragging.current);
    align();
    ...
    showTimestamps
      ? (
        <div className={cn("flex items-start gap-2", dicht ? denseStepClasses : stepClasses)} key={`zeit-${key}`}>
          <time className="w-[34px] flex-none pt-0.5 text-[10.5px] tabular-nums text-muted-foreground opacity-85">{zeitVon(at)}</time>
  ...
  const verlauf = (
    <div className={cn("@container/chat relative flex min-h-0 flex-1 flex-col", className)}>
      <div
        aria-label="Chatverlauf"
        className={cn(
          "min-h-0 flex-1 cursor-default overflow-y-auto text-[13px] text-foreground",
      ...
      >
        {visible.length === 0 && !running ? (
          ...
          <div
            ...
            className={cn(
              "mx-auto w-full max-w-[760px] px-5 pt-2 @max-[620px]/chat:px-3",
              "pb-[calc(var(--composer-height,0px)_+_max(3.25em,40px))]",
              "in-data-[surface=material]:px-3 in-data-[surface=material]:[&>*:first-child]:mt-0",
            )}
            ref={threadRef}
          >
            {bloecke}
      ...
      {!amEnde && (
        <div className="pointer-events-none absolute inset-x-0 bottom-[calc(var(--composer-height,0px)_+_12px)] z-10 flex justify-center">
          <Button aria-label={alleTexte.jumpToEnd} className="pointer-events-auto rounded-full border border-primary-foreground/20 shadow-md" size="icon" title={alleTexte.jumpToEnd} variant="default" onClick={() => {
            if (scrollBereich.current) scroll.jump(scrollBereich.current);
          }}>
            <ChevronDownIcon />
          </Button>
        </div>
  ...
  return <MarkdownLinks onLinkClick={onLinkClick}>{verlauf}</MarkdownLinks>;
}
...
function Bubble({
    ...
    return (
      <div className={schritt}>
        <QuestionCard
          ...
          text={message.text}
          onAnswer={onAnswerQuestion ? (text) => onAnswerQuestion(message.question!.callId, text) : undefined}
        />
      </div>
    );
  }
  ...
  if (message.role === "user" && !message.bubble && !plain) {
    return (
      <div className={cn(schritt, "flex justify-end")} data-message="user">
        <div className={cn(bubbleBodyClasses, "group rounded-[10px_10px_4px_10px] border border-border bg-secondary text-foreground [--scroll-cover:var(--secondary)]",
          "in-data-[surface=material]:rounded-[9px] in-data-[surface=material]:border-glass-edge/20 in-data-[surface=material]:bg-primary/15 in-data-[surface=material]:text-foreground")}>
          <Markdown text={message.text} />
          <MessageAttachments message={message} />
  ...
  if (!inhalt && !message.attachments?.length) {
    return null;
  }

  if (message.bubble && !plain) {
    return (
      <div data-chat-text-cursor={message.role === "assistant" && message.textCursor ? JSON.stringify(message.textCursor) : undefined}
        className={cn(schritt, "flex", message.bubble.side === "end" ? "justify-end" : "justify-start")}
        data-message="bubble" data-side={message.bubble.side === "end" ? "end" : "start"}>
        <div className={cn(bubbleBodyClasses, "group rounded-lg text-white [--scroll-cover:var(--primary)]")} data-tone="on-color" style={{ background: message.bubble.color }}>
          {message.bubble.label && <span className="mb-[5px] table rounded-full border border-white/30 bg-black/15 px-[7px] py-px text-[0.72rem] leading-[1.45] font-semibold tracking-[0.02em]">{message.bubble.label}</span>}
  ...
  return (
    <div data-chat-text-cursor={message.role === "assistant" && message.textCursor ? JSON.stringify(message.textCursor) : undefined}
      className={cn(schritt, "group relative max-w-[92%] leading-[1.625] text-foreground @max-[620px]/chat:max-w-full")} data-message="answer">
      <Markdown text={message.text} streaming={message.role === "assistant" && !message.closed} />
      <MessageAttachments message={message} />
      <MessageCopy message={message} />
    </div>
...
function MessageCopy({ message }: { message: Message }) {
  ...
  if (message.role !== "user" || !message.text.trim()) return null;
  const label = status === "copied" ? "Kopiert" : status === "failed" ? "Nachricht konnte nicht kopiert werden" : "Nachricht kopieren";
  const copy = () => {
  ...
  return (
    <div className={messageActionsClasses}>
      <Button aria-label={label} className="text-inherit" size="icon-sm" title={label} variant="ghost" onClick={copy}>
