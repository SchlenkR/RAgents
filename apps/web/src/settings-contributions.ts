...
export const toolKindLabel = (kind: SettingsTool["kind"]): string => {
  if (kind === "agent-builtin") return "Agent Builtin";
  if (kind === "agent-extension") return "Agent-Extension";
...
const webContributionsOf = (plugin: WebPlugin): readonly WebContribution[] => [
  ...
  flagged("Rückfragen-Beantworter", plugin.questionResponder !== undefined),
