...
/** Das Menü rechts oben in der Spalte: Einstellungen, Server im Browser und Abmelden hinter einem Knopf. */
export function ColumnMenu({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  return <>
    <Button aria-controls={open ? panelId : undefined} aria-expanded={open} aria-haspopup="dialog" aria-label="Menü der Spalte" className="self-center" onClick={() => setOpen((value) => !value)} ref={buttonRef} size="icon-lg" title="Menü" variant="ghost"><MoreVerticalIcon /></Button>
    <Popover onOpenChange={(next, details) => { if (!next && pressedAnchor(details)) return; setOpen(next); }} open={open}>
      <PopoverContent align="end" anchor={buttonRef} aria-label="Menü der Spalte" className="w-[220px] gap-0 p-1 text-[0.8rem]" collisionPadding={8} dim id={panelId} role="dialog" side="bottom" sideOffset={4}>
