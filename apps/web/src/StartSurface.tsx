...
export function StartSurface({ registry, session, initialEntryId }: {
  ...
  useEffect(() => {
    ...
    const entry = registry.scriptEntries.find((candidate) => candidate.id === initialEntryId);
    if (!entry || !canStartEntry(access, entry.id)) {
      setError("Dieses Sample ist im aktuellen Profil nicht verfügbar.");
      return;
    }
...
export function openStartEntry(entry: StartEntry, registry: PluginRegistry, modal: ModalController,
  ...
  const proceed = (value: JsonValue | null) => {
    ...
    modal.open({ title: "Auftrag vorbereiten", subtitle: entry.title,
      render: () => <RunPreparationChat initialPrompt={value ?? entry.prompt} skillName={entry.skill}
...
export function StartEntryLibrary({ skills, scripts, launchDisabled, onOpen, hasGuide, technical = true }: {
  ...
  const filtered = sorted.filter((entry) => (!tag || entry.tags?.includes(tag))
    && `${entry.title} ${entry.description} ${entry.action === "skill" ? entry.category : kindLabel(entry)} ${entry.owner} ${(entry.tags ?? []).join(" ")}`
  ...
  const items: ListDetailItem[] = filtered.map((entry) => ({ id: entry.id, title: entry.title,
    description: entry.description, group: entry.action === "skill" ? entry.category : technical ? "Run-Scripts" : "Abläufe",
