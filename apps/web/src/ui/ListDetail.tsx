...
export function ListDetail({ label, items, selectedId, onSelect, disabled = false, toolbar,
  ...
  const [mobileDetailId, setMobileDetailId] = useState<string>();
  ...
  const showMobileDetail = selected !== undefined && mobileDetailId === selected.id;

  useLayoutEffect(() => {
    const request = focusRequest.current;
    if (!request) return;
    focusRequest.current = null;
    if (request.target === "detail" && (!showMobileDetail || selected?.id !== request.id)) return;
    const target = request.target === "detail" ? detailRef.current : buttons.current.get(request.id) ?? listRef.current;
    target?.focus({ preventScroll: true });
    target?.scrollIntoView({ block: "nearest", inline: "nearest" });
  }, [mobileDetailId, selected?.id, showMobileDetail]);
  ...
  const returnToList = () => {
    ...
    setMobileDetailId(undefined);
  };

  return <section aria-label={label} className={cn("group/list-detail flex min-h-0 min-w-0 flex-auto flex-col", className)}
    data-detail={showMobileDetail}>
    {toolbar !== undefined && <div className="flex flex-none flex-wrap items-center gap-2.5 pb-4.5">{toolbar}</div>}
    <div className="grid min-h-0 min-w-0 flex-auto grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] grid-rows-[minmax(0,1fr)] gap-6 max-[700px]:grid-cols-[minmax(0,1fr)] max-[700px]:gap-0">
      <div aria-label={label} className={cn(paneClasses, focusRing, "pt-0.5 pr-1.5 pb-3 max-[700px]:group-data-[detail=true]/list-detail:hidden")}
            ...
            <ul className="grid grid-cols-[minmax(0,1fr)] gap-1 *:min-w-0">
              {entries.map((item) => <li key={item.id}>
                <button aria-controls={`${prefix}-detail`} aria-pressed={item.id === selectedId}
                  ...
                  {item.meta !== undefined && <span className="max-w-[30%] flex-none text-[0.62rem] leading-snug text-muted-foreground [overflow-wrap:anywhere] max-[700px]:max-w-[24%] max-[700px]:text-[0.6rem]">{item.meta}</span>}
      ...
      <section aria-label={detailLabel} className={cn(toneVariables, focusRing,
        ...
        "max-[700px]:hidden max-[700px]:group-data-[detail=true]/list-detail:flex")}
        data-tone={selected?.tone ?? "neutral"}
        id={`${prefix}-detail`} ref={detailRef} tabIndex={-1}>
        {selected ? <>
          <div className="hidden flex-none border-b border-border-soft px-3.5 py-2.5 max-[700px]:flex"><Button aria-label="Zur Auswahl" className="rounded-full" onClick={returnToList} size="icon" variant="outline"><ArrowLeftIcon /></Button></div>
          {detailHeader !== undefined && <header className="flex-none border-b border-border-soft bg-(--tone-soft) px-6 py-6 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 max-[700px]:p-4.5">{detailHeader}</header>}
          <div aria-label={`${detailLabel}: Inhalt`} className={cn(paneClasses, focusRing, "flex-1 px-6 py-6 text-[0.8rem] leading-[1.7] [overflow-wrap:anywhere] [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 max-[700px]:p-4.5")} role="region" tabIndex={0}>{children}</div>
          {detailFooter !== undefined && <footer className="flex flex-none flex-wrap items-center gap-2.5 border-t border-border-soft px-6 py-4 max-[700px]:px-4.5 max-[700px]:py-3.5">{detailFooter}</footer>}
