...
export const homepageSamples = [
  { id: "word-game", title: "Word game" },
  { id: "learning-afternoon", title: "Learning afternoon" },
] as const;
  ...
  for (const { id, title } of homepageSamples) {
    ...
    const result = await build({
      ...
      alias: {
        ...
        "@ragents/client/ui": path.join(repoRoot, "apps/web/src/actor-programs/client-ui/index.tsx"),
      },
...
<body>
<p class="sample-preview-notice">Preview with example data.</p>
<main id="root"><p>This preview requires JavaScript. The sample and its sources are available in the <a href="reference.html#start-ragents.reference.${id}" target="_top">reference</a>.</p></main>
</body>
</html>
`);
    outputs.set(`${id}.js`, javascript.trimEnd() + "\n");
