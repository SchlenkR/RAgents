...
import { assertPublicOutput, corePluginIds, publicPackageFiles } from "./homepage-catalog.js";
import { readHomepageUiContracts } from "./homepage-ui-contracts.js";
import { readClientUiComponentNames, readClientUiContractFiles } from "../../plugins/ragents.actor-programs/server/client-contracts.js";
...
test("Profilinventar liest ausschließlich literale IDs und führt die Konfiguration nicht aus", () => {
  ...
  try {
    writeFileSync(path.join(root, "ragents.config.core.ts"), 'throw new Error("nicht ausführen"); const config = { host: { PLUGINS: ["ragents.example"] }, token: process.env.SECRET };');
    assert.deepEqual(corePluginIds(root), ["ragents.example"]);
    for (const plugins of ['["private.product"]', '["ragents.example", "ragents.example"]', 'loadPlugins()', '[]', '[...otherPlugins]']) {
      writeFileSync(path.join(root, "ragents.config.core.ts"), `const config = { host: { PLUGINS: ${plugins} } };`);
      assert.throws(() => corePluginIds(root));
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
});

test("öffentliche Artefakte lehnen private Produktnamen und lokale Pfade ab", () => {
  for (const value of ["plugins/private.product", "plugins/Private.Product", "/Users/example/repo", "/private/tmp/example", "/tmp/example", "OPENROUTER_VSCODE_APIKEY"]) {
...
function uiFixture() {
  ...
  const ui = "plugins/ragents.actor-programs/client-ui/";
