...
const contracts: Contract[] = [
  ...
  { id: "run", file: "plugins/ragents.actor-programs/server/app-project.ts", name: "RunContext", kind: "embedded" },
  { id: "app", file: "plugins/ragents.actor-programs/server/client-compiler.ts", name: "AppContext", kind: "embedded" },
  { id: "appState", file: "plugins/ragents.actor-programs/server/client-compiler.ts", name: "AppStateView", kind: "embedded" },
  { id: "appCapabilities", file: "plugins/ragents.actor-programs/server/client-compiler.ts", name: "AppCapabilities", kind: "embedded" },
  { id: "chat", file: "plugins/ragents.actor-programs/client-ui/contracts.d.ts", name: "ChatConnection" },
  { id: "appPackage", file: "plugins/ragents.actor-programs/server/app-project.ts", name: "appPackageSchema", kind: "schema" },
  { id: "appContract", file: "plugins/ragents.actor-programs/server/app-project.ts", name: "appContractSchema", kind: "schema" },
  { id: "appAction", file: "plugins/ragents.actor-programs/server/app-project.ts", name: "actionSchema", kind: "schema" },
  ...
  { id: "lsp", file: "apps/server/src/plugin-support/language-server/host.ts", name: "LanguageServerAdapter" },
];
...
] satisfies readonly ProfileUser[];`, ["profileUser.id", "profileUser.label", "profileUser.password", "profileUser.rights", "profileUser.startEntries", "environmentReference.kind", "environmentReference.name"], [
...
host.lifecycle({
  ...
  entry("tools", "Serverbeiträge", "Eine typisierte Run-Funktion", "Ein Plugin stellt eine Funktion einmal mit Beschreibung, Eingabe, Ergebnis und Implementierung bereit. Ein LLM verwendet sie in einem TypeScript-Snippet; ein dauerhafter Actor ruft dieselbe Funktion mit derselben API auf.", "register(host); Type aus typebox, Helfer aus @aicontainer/ragents.", `
...
const result = await host.invokeOperation(
  ...
  entry("agent-runtime", "Serverbeiträge", "AgentSession-Extensions beitragen", "Jeder KI-Agent hat eine eigene AgentSession, die seinen Modellkontext und seine Werkzeugaufrufe verwaltet. Eine Agent-Extension erweitert diese Sitzung, etwa um ein zusätzliches Werkzeug. Das Plugin stellt dafür die Erzeugungsfunktion und die Werkzeugbeschreibungen bereit.", "register(host); InlineExtension kommt aus @aicontainer/agent, Type aus typebox.", `
...
});`, ["host.startEntries", "start.id", "start.title", "start.description", "start.order", "start.guide", "start.tags"], [
...
const tabs = {
  ...
  entry("web-canvas", "Web-Beiträge", "Eine zentrale Canvas-Ansicht", "Die zentrale Arbeitsfläche heißt in RAgents Canvas und zeigt unter anderem die Karten der Beteiligten. Ein Plugin kann die Darstellung dieser Fläche übernehmen. Dafür erhält es die vorhandenen Funktionen für Chats, Navigation, Kartenabschnitte und zusätzliche Elemente.", "Eigenschaften eines WebPlugin mit dem vorgeschriebenen statischen Tab.", `
const canvasUi = {
  id: "ragents.example",
  ...
  entry("web-canvas-elements", "Web-Beiträge", "Elemente auf der vorhandenen Fläche", "Ein Plugin kann zusätzliche Elemente auf der vorhandenen Arbeitsfläche, dem Canvas, anzeigen. Es liefert dazu die Beschreibung des Elements und seine Darstellung. Das Element ist eine Oberfläche und bearbeitet selbst keine Aufträge.", "Eigenschaften eines WebPlugin.", `
const elements = {
  id: "ragents.example",
  canvasElements: [{
...
const metadata = {
  ...
  entry("web-attention", "Web-Beiträge", "Aufmerksamkeit und Rückfragen", "Ein Plugin kann markieren, dass eine Unterhaltung Aufmerksamkeit braucht. Für Rückfragen verbindet ein Question-Responder die Antwort in der Oberfläche mit der zuständigen Serverfunktion. Die Markierung und die Antwortzustellung sind getrennte Beiträge.", "Eigenschaften eines WebPlugin; answerQuestion ist der eigene geprüfte HTTP-Client.", `
const interaction = {
  id: "ragents.example",
...
} satisfies WebPlugin;`, ["web.attention", "web.questionResponder"], ["Es darf höchstens einen Question-Responder geben. Neben einem bereits vorhandenen Rückfragenplugin wird kein zweiter registriert. Attention erzeugt selbst keine Frage oder Hintergrundarbeit."], "tsx"),
...
test("erhält vorhandene Einträge", async () => {
  ...
  entry("app-placements", "Actor-Programme", "Views auf dem Canvas", "Eine aktivierte View erscheint beim zugehörigen Actor auf der Arbeitsfläche. Der Benutzer kann sie im Host vergrößern; mehrere Views zeigen denselben Actor-Zustand.", "Ein Eintrag in package.json.ragents.views.", `
...
});`, ["workspace.resolve", "workspace.describe", "workspace.toolNaming", "resolver.optionId", "resolver.resolve"], [
...
};`, ["lsp.id", "lsp.label", "lsp.languages", "lsp.rootDescription", "lsp.resolveRoot", "lsp.launch", "lsp.open"], [
