...
function guideLink(href: string, source: string): string {
  ...
  if (/^(?:[a-z][a-z\d+.-]*:|\/)/i.test(href)) throw new Error(`Invalid guide link: ${href}`);
...
export function guideExcerpt(markdown: string, id: string): string {
  ...
  for (const line of markdown.split("\n")) {
    ...
    if (boundary[1]) {
      if (active !== boundary[2]) throw new Error(`Mismatched guide section end: ${boundary[2]}`);
      if (active === id) {
        if (!chunk.join("\n").trim()) throw new Error(`Empty guide section: ${id}`);
        chunks.push(chunk.join("\n").trim());
      }
      active = undefined;
      chunk = [];
    } else {
      if (active) throw new Error(`Nested guide sections: ${active}, ${boundary[2]}`);
      if (!guideChapters.some((chapter) => chapter.id === boundary[2])) throw new Error(`Unknown guide chapter: ${boundary[2]}`);
      active = boundary[2];
    }
  }
  if (active) throw new Error(`Unclosed guide section: ${active}`);
  if (!chunks.length) throw new Error(`Missing guide section: ${id}`);
  ...
  const parser = new Marked({
    ...
    renderer: {
      heading({ tokens, depth, text }) {
        if (depth < 2) throw new Error(`Guide content must start at heading level 2: ${source}`);
      ...
      html() { throw new Error(`Express guide content as Markdown instead of HTML: ${source}`); },
    },
    walkTokens(token) {
...
const chapterNavigation = (current?: string) => `<nav class="guide-chapters" aria-label="Guide navigation"><a href="guide.html"${current ? "" : ' aria-current="page"'}>Overview</a>${navigationGroups.map(group => `<div class="guide-nav-group"><p>${group.title}</p>${group.chapters.map(id => {
