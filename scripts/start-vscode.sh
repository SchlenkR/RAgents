...
profile=""
case "$target" in
    http://*|https://*) url="$target"; connection_name="$target" ;;
    */*|*.ts) config_file="$(cd "$(dirname "$target")" && pwd)/$(basename "$target")"; profile="$config_file"; connection_name="$(basename "$config_file" .ts)"; connection_name="${connection_name#ragents.config.}" ;;
    *) profile="$target"; connection_name="$target"; config_file="$root/ragents.config.$target.ts" ;;
esac
if [ -n "$profile" ]; then
...
settings="$user_dir/User/settings.json" url="$url" name="$connection_name" node --input-type=module -e '
...
const next = {
    ...
    ...current,
    "ragents.connections": [{ name: process.env.name, url: process.env.url }],
};
writeFileSync(file, JSON.stringify(next, null, 2) + "\n");
'

echo "== VS-Code-Testinstanz gegen $url (Benutzerordner: $user_dir)"
echo "   RAgents-Symbol in der Aktivitätsleiste öffnen; das RAgents-Panel liegt in der zweiten Seitenleiste."
