...
Host-Bausteine importiert ein Plugin über die Pakete @aicontainer/server, @aicontainer/web und @aicontainer/ragents, andere Plugins über @aicontainer/plugins/<id>. Damit darf der Plugin-Ordner an beliebiger Stelle liegen; das Profil nennt ihn per Kennung oder Pfad.
...
import type { RAgentsConfig } from "@aicontainer/server/config-definition.js";
...
```typescript
import { env, type ProfileUser } from "@aicontainer/server/config-definition.js";
...
Ohne users-Export gibt es keine Anmeldung. Ein optionaler anonymousUser kann den Zugang trotzdem einschränken. Eine leere users-Liste ist ein Startfehler. Passwörter dürfen Klartext oder env-Referenzen sein und werden niemals im Browser veröffentlicht.

Rechte sind exakte Strings. Nur der einzelne Wert * bedeutet alle Rechte; Teilmuster und Vererbung gibt es nicht. Ein Profil teilt seine Runs mit allen dafür leseberechtigten Benutzern. runs.write erlaubt Chat und App-Aktionen in bestehenden Runs. runs.create ergänzt freie Runs und Startoptionen. Ohne dieses Recht begrenzt startEntries die freigegebenen Run-Scripts; runs.inspect schützt technische Ansichten. Diese Rechte ersetzen keine Sandbox für nativen Code.
...
Vertragsfelder: profileUser.id, profileUser.label, profileUser.password, profileUser.rights, profileUser.startEntries, environmentReference.kind, environmentReference.name.
...
```typescript
import type { ProfileAnonymousUser } from "@aicontainer/server/config-definition.js";
...
### Eine Erweiterungsroute mit eigenen Rechten
...
Plugins können Daten für die ganze Anwendung oder für eine einzelne Unterhaltung speichern. Die Anwendung stellt jedem Plugin dafür eigene Ablagepfade bereit und ordnet sie seiner Kennung zu.
...
Einsatzort: Gemeinsamer Vertrag plus Registrierung; serviceToken kommt aus @aicontainer/ragents.
...
Ein Plugin kann bei Anwendungsstart, beim Stoppen oder Löschen einer Unterhaltung und beim Herunterfahren eigene Funktionen ausführen. So lassen sich seine Hintergrunddienste und Ressourcen passend starten und aufräumen.
...
Einsatzort: register(host); Type aus typebox, Helfer aus @aicontainer/ragents.
...
### AgentSession-Extensions beitragen

Jeder KI-Agent hat eine eigene AgentSession, die seinen Modellkontext und seine Werkzeugaufrufe verwaltet. Eine Agent-Extension erweitert diese Sitzung, etwa um ein zusätzliches Werkzeug. Das Plugin stellt dafür die Erzeugungsfunktion und die Werkzeugbeschreibungen bereit.

Einsatzort: register(host); InlineExtension kommt aus @aicontainer/agent, Type aus typebox.

```typescript
const description = "Gibt einen Text unverändert zurück.";
const extension: InlineExtension = {
  name: "example-echo",
  factory: (api) => {
    api.registerTool({
      name: "example_echo", label: "Echo", description,
...
host.agentRuntime({
  id: "ragents.example.extensions", global: [extension],
  tools: [{ name: "example_echo", description, scope: "per-agent",
    availability: "always", availabilityDetail: "In dieser Agent-Session verfügbar." }],
});
```

global bedeutet eine allgemein beigetragene Factory, keine einzige globale AgentSession. resolve(context) ergänzt Factories gezielt je Agent; der Kontext enthält Run, Agent, Audience und Arbeitsverzeichnis.

Falls die Extension Werkzeuge registriert, muss tools deren per-agent-Deskriptoren vollständig angeben. Änderungen gelten im Lebenszyklus der AgentSession, nicht als freie globale Hintergrundaktivität.

Vertragsfelder: host.agentRuntime.

### Modelle und Agentenprofile

Der Modellkatalog nennt die KI-Modelle, die für Agenten auswählbar sind. Ein Agentenprofil kombiniert ein Modell mit Einstellungen wie Denktiefe und Ausführungsgrenzen. Solche Agentenprofile beschreiben einzelne Agenten; das Anwendungsprofil stellt die gesamte Installation zusammen.
...
### Skills und Run-Scripts als Einstieg

Die Startfläche bietet Vorlagen für eine neue Unterhaltung an. Ein Skill verbindet einen bearbeitbaren Startauftrag mit einer Arbeitsanleitung und optionalen Dateien. Ein Run-Script liefert einen programmierten Aufbau. Plugins melden beide Arten über denselben Vertrag an.
...
Vertragsfelder: host.startEntries, start.id, start.title, start.description, start.order, start.guide, start.tags.

### Startwerte prüfen und einfrieren

Startoptionen sind Werte, die vor einer neuen Unterhaltung gewählt werden, etwa das Modell. Das Plugin legt erlaubte Werte, einen Standardwert und die Prüfung der Auswahl fest. Beim Start wird der gewählte Wert für die Unterhaltung gespeichert und fixiert.
...
Ein Plugin kann kurze Zusatzangaben zu einer Unterhaltung liefern, etwa einen Bearbeitungsstatus. Solche Metadaten stehen der Oberfläche zur Anzeige zur Verfügung. Die zugrunde liegenden Fachdaten bleiben beim Plugin.
...
Die TypeScript-Laufzeit führt die programmierten Abläufe einer Unterhaltung aus. Der Server bindet sie über den Beitrag script ein. Dieser stellt die Ausführung und die verfügbaren Programmierfunktionen bereit.
...
### Eine Erweiterungsansicht ausblenden oder nur lesbar zeigen
...
Einsatzort: React-Komponente einer Extension; useAccess kommt aus dem Web-Host und accessMode aus dem gemeinsamen Rechtevertrag.
...
Ein Plugin kann einen eigenen Reiter mit Symbol, Inhalt und optional einer Statusmarkierung ergänzen. Feste Reiter sind immer Teil seines Angebots. Dynamische Reiter entstehen passend zum Zustand der geöffneten Unterhaltung.
...
Auch dynamische Tab-IDs müssen eindeutig sein. Ein Canvas-Tab muss statisch registriert sein.

Vertragsfelder: web.workspaceTabs, web.workspaceTabsFor.

### Eine zentrale Canvas-Ansicht

Die zentrale Arbeitsfläche heißt in RAgents Canvas und zeigt unter anderem die Karten der Beteiligten. Ein Plugin kann die Darstellung dieser Fläche übernehmen. Dafür erhält es die vorhandenen Funktionen für Chats, Navigation, Kartenabschnitte und zusätzliche Elemente.

Einsatzort: Eigenschaften eines WebPlugin mit dem vorgeschriebenen statischen Tab.

```tsx
const canvasUi = {
  id: "ragents.example",
  workspaceTabs: [{ id: "ragents.example.canvas", label: "Fläche", order: 100,
    Icon: () => <span>F</span>, Panel: () => <p>Auswahl der Fläche</p> }],
  canvas: {
    tabId: "ragents.example.canvas",
    Center: ({ renderChat }) => <div>{renderChat()}<p>Eigene Fläche</p></div>,
  },
} satisfies WebPlugin;
```

Es gibt höchstens einen Canvas-Beitrag. Das Beispiel ersetzt die zentrale Fläche; es ergänzt nicht automatisch die bestehende Orchestrierungsfläche.
...
Vertragsfelder: web.canvas.

### Elemente auf der vorhandenen Fläche

Ein Plugin kann zusätzliche Elemente auf der vorhandenen Arbeitsfläche, dem Canvas, anzeigen. Es liefert dazu die Beschreibung des Elements und seine Darstellung. Das Element ist eine Oberfläche und bearbeitet selbst keine Aufträge.
...
const elements = {
  ...
  canvasElements: [{
    ...
    select: () => [{ id: "example-note", title: "Hinweis", width: 320, height: 180, collapsedHeight: 34, visible: true }],
    Element: ({ definition }) => <p>{definition.title}</p>,
  }],
} satisfies WebPlugin;
```

Definitionen nennen id, width und height; title ist optional. visible: false nimmt ein Element aus Layout und Darstellung, ohne eine fehlende explizite Layoutreferenz zu melden. Ohne visible ist es sichtbar. collapsedHeight gibt optional die Layout-Höhe im eingeklappten Zustand an; sie muss endlich und größer als null sein. anchorActorId, entity, resizable mit Größenlimits und eigene data sind optional. Das Layout entscheidet über die Platzierung; der Beitrag allein erzeugt keine Actor-Identität. Die persönliche Canvas-Ansicht erkennt Besitzer an anchorActorId, auch bei visible: false. Ihre Actor-Karten sind standardmäßig ausgeblendet und bleiben über die Actorliste erreichbar; sichtbare Elemente bleiben auch bei ausgeblendeter Ankerkarte auf der Fläche.

Vertragsfelder: web.canvasElements, canvasElement.id, canvasElement.visible, canvasElement.title, canvasElement.width, canvasElement.height, canvasElement.collapsedHeight, canvasElement.anchorActorId, canvasElement.entity, canvasElement.resizable, canvasElement.data.

### Abschnitte an Actor-Karten

Die Beteiligten einer Unterhaltung heißen Actors und werden auf der Arbeitsfläche als Karten dargestellt. Ein Plugin kann diese Karten um eigene Abschnitte ergänzen, etwa für Dokumente oder einen Status.
...
### Laufdaten und React-Kontext

Mehrere Oberflächenbeiträge eines Plugins können gemeinsame Daten zur geöffneten Unterhaltung benötigen. Ein SessionProvider reicht diese über React-Kontext weiter. Mit needsRunView fordert das Plugin zusätzlich den vom Server bereitgestellten Zustand der Unterhaltung an.
...
Ein Plugin kann Informationen für die ganze Anwendung oder für die gerade geöffnete Unterhaltung anzeigen. Beiträge für die ganze Anwendung stehen in der Übersicht oder in der globalen Kopfzeile; Beiträge zur Unterhaltung folgen dem ausgewählten Run. Dafür gibt es overviewPanels mit einer Platzierungswahl und sessionHeaders für die Titelleiste oder die Leiste oben am Canvas.
...
category: models zeigt den Beitrag unter Modelle, appearance unter Darstellung. Ohne category bleibt er bei der zugehörigen Extension. Die Formularbereiche laden unabhängig vom technischen Beitragskatalog.
...
Vor einer neuen Unterhaltung können Startoptionen direkt gewählt oder in einem Einrichtungsdialog gesammelt werden. Das Plugin liefert dafür die Oberfläche und verbindet ihre Werte mit dem jeweiligen Serververtrag. Ein Leitfaden ist ein solcher Dialog für einen vorbereiteten Ablauf.
...
Zusatzangaben zu einer Unterhaltung können in der Run-Liste und in ihrer Kopfzeile erscheinen. Das Plugin erhält die Daten der Unterhaltung und den Anzeigeort. So kann es denselben Status je nach Platz kurz oder ausführlicher darstellen.
...
### Aufmerksamkeit und Rückfragen

Ein Plugin kann markieren, dass eine Unterhaltung Aufmerksamkeit braucht. Für Rückfragen verbindet ein Question-Responder die Antwort in der Oberfläche mit der zuständigen Serverfunktion. Die Markierung und die Antwortzustellung sind getrennte Beiträge.
...
const interaction = {
  ...
  questionResponder: (session, callId, payload) => answerQuestion(session.id, callId, payload),
} satisfies WebPlugin;
```

Es darf höchstens einen Question-Responder geben. Neben einem bereits vorhandenen Rückfragenplugin wird kein zweiter registriert. Attention erzeugt selbst keine Frage oder Hintergrundarbeit.

Vertragsfelder: web.attention, web.questionResponder.
...
Ereignisse melden, was in einer Unterhaltung geschehen ist, etwa dass ein Agent einen Beitrag abgeschlossen hat. Ein Abonnement, die Subscription, stellt passende neue Ereignisse einem Beteiligten als Nachricht zu. Ein programmierter Vermittler kann daraufhin das Ergebnis weitergeben oder den nächsten Arbeitsschritt anstoßen.
...
Ein Run-Script ist ein vollständiges Actor-Programmpaket für den Start einer Unterhaltung. Der Host aktiviert es mit demselben Compiler und denselben Fachtests wie ein während des Runs geschriebenes Programm.
...
### Views auf dem Canvas

Eine aktivierte View erscheint beim zugehörigen Actor auf der Arbeitsfläche. Der Benutzer kann sie im Host vergrößern; mehrere Views zeigen denselben Actor-Zustand.

Einsatzort: Ein Eintrag in package.json.ragents.views.

```json
{ "id": "main", "title": "Liste", "client": "src/client.tsx", "width": 480, "height": 320 }
```

actor_view_set_visibility verwendet Paketname/Viewname oder einen eindeutigen Titel. Die View-Kennung ist innerhalb des Pakets eindeutig. Ohne Größenangabe verwendet der Host 480 mal 320 Pixel und den zugehörigen Actor als Anker.
...
Der Koordinator ist der zentrale KI-Ansprechpartner einer Unterhaltung und kann Aufgaben an weitere Agenten verteilen. Die Produktpolitik legt seinen Start, die Rollen der Beteiligten und ihre Anweisungen fest. Eine eigene Anwendung stellt diese Regeln als Dienst bereit.
...
Ein Arbeitsbereich legt fest, in welchen Verzeichnissen die Agenten einer Unterhaltung mit Dateien und Prozessen arbeiten. Der Dienst WorkspaceRuntime löst diesen Bereich auf und beschreibt ihn. Ein optionaler WorkspaceResolver kann die Zuordnung anpassen, ohne den ganzen Dienst zu ersetzen.
...
resolve der vollständigen WorkspaceRuntime liefert eine SessionWorkspace mit cwd, currentRoot, ensureWritable und runOperation; optional kommen gitEnv, gitConfig und extraEnv hinzu. describe liefert Modus und Verzeichnismuster; toolNaming kann die vorhandenen Dateiwerkzeuge benennen.

Ein Resolver erhält runId, directory, choice und emitSystem. optionId verbindet die Auswahl mit einer registrierten Startoption. Der gezeigte Resolver behält das vom Host vorbereitete Run-Verzeichnis bei.
...
Vertragsfelder: workspace.resolve, workspace.describe, workspace.toolNaming, resolver.optionId, resolver.resolve.

### Dokumentablage und Git-Ansichten

Die Dokumentablage ordnet jeder Unterhaltung ein Verzeichnis für ihre Dateien zu. Eine zusätzliche Git-Ansicht kann den Branch, Änderungen und Dateiinhalte eines Arbeitsbereichs liefern. Plugins stellen diese Dienste bereit, damit andere Beiträge dieselben Daten verwenden können.
...
Einsatzort: Ein einfacher Testdriver ohne Modellaufruf; AgentDriver kommt aus @aicontainer/ragents.
...
languages ordnet Dateiendungen Sprachkennungen zu. resolveRoot prüft das Projektziel, launch liefert den Prozessstartvertrag für den Sandbox-Kontext und open ergänzt bei Bedarf serverspezifische Öffnungsschritte.
...
Vertragsfelder: lsp.id, lsp.label, lsp.languages, lsp.rootDescription, lsp.resolveRoot, lsp.launch, lsp.open.
...
- settings.read: Profil, Konfiguration und Erweiterungen ansehen.

- settings.write: Einstellungen und externen Zugang ändern.

- ragents.overseer.read: Den übergeordneten Koordinator und seine Modellauswahl ansehen.

- ragents.overseer.write: Dem übergeordneten Koordinator Aufträge geben und sein Gespräch zurücksetzen.
...
Quelle im Repository: plugins/ragents.actor-programs/server/app-project.ts
...
Quelle im Repository: plugins/ragents.actor-programs/server/client-compiler.ts
...
Quelle im Repository: plugins/ragents.actor-programs/client-ui/contracts.d.ts
...
```typescript
{
  ...
  views: Type.Optional(Type.Array(Type.Object({
    ...
    width: Type.Optional(Type.Integer({ minimum: 280, maximum: 960 })),
    height: Type.Optional(Type.Integer({ minimum: 180, maximum: 720 })),
...
Quelle im Repository: apps/server/src/plugin-support/language-server/host.ts
