...
import { isPendingActorInput, actorStatePluginId } from "./model.ts";
...
const actorOf = (state: RunState, actorId: string): Actor => {
...
const toolCallOf = (
    ...
    const call = context.toolCalls.get(toolCallKey(turnId, toolCallId));

    if (!call)
        throw new Error(`Tool call ${toolCallId} does not exist in turn ${turnId}.`);

    return call;
};

const runningToolCallsOf = (context: EventSemanticContext, turnId: string) =>
    [...context.toolCalls.values()].filter((call) => call.turnId === turnId && call.status === "running");
...
const assertSubscriptionSource = (
    ...
    if (!subscription.includeSelf && source.actorId === subscription.subscriberId)
        throw new Error(`Source event ${sourceEventId} is excluded by subscription ${subscription.id}.`);

    if (subscription.sourceActorIds && !subscription.sourceActorIds.includes(source.actorId))
    ...
    if (subscription.sourceActorKinds) {
        const sourceActor = actorOf(state, source.actorId);
...
export const assertEventSemantics = (
    ...
    switch (event.type) {
        ...
        case "turn.finished": {
            ...
            const openCalls = runningToolCallsOf(context, turn.id);

            if (openCalls.length > 0)
                throw new Error(
                    `Turn ${turn.id} still has running tool calls: ${openCalls.map((call) => call.toolCallId).join(", ")}.`,
                );
            break;
        }
        ...
        case "tool.call.started": {
            const turn = runningTurnOf(state, event.payload.turnId);
            assertTurnAuthor(turn, event);
            const key = toolCallKey(turn.id, event.payload.toolCallId);

            if (context.toolCalls.has(key))
        ...
        case "tool.call.failed": {
            const turn = runningTurnOf(state, event.payload.turnId);
            assertTurnAuthor(turn, event);
            const call = toolCallOf(context, turn.id, event.payload.toolCallId);

            if (call.status !== "running")
                throw new Error(`Tool call ${call.toolCallId} in turn ${call.turnId} is already ${call.status}.`);

            if (call.name !== event.payload.name)
                throw new Error(`Tool call ${call.toolCallId} started as ${call.name}, not ${event.payload.name}.`);
            break;
        }
        ...
        case "tool.call.source": {
            ...
            if (call.status !== "running")
                throw new Error(`Tool call ${call.toolCallId} in turn ${call.turnId} is already ${call.status}.`);
            if (call.sourceRecorded)
                throw new Error(`Tool call ${call.toolCallId} in turn ${call.turnId} already has a source snapshot.`);
            break;
        }
        ...
        case "plugin.state-replaced": {
            ...
            if (event.payload.scope.kind === "actor") {
                actorOf(state, event.payload.scope.actorId);
        ...
        case "action.proposed": {
            ...
            if (event.payload.kind === "action" && !hasRunCapability(author, "action.propose"))
        ...
        case "action.resolved": {
            ...
            if (actorOf(state, event.actorId).kind !== "human")
                throw new Error(`Action ${action.id} was not resolved by a human actor.`);

            if (event.payload.decision === "approved" && action.input?.required && !event.payload.response?.trim())
                throw new Error(`Action ${action.id} requires a response.`);
            break;
        }
...
export const recordEventSemantics = (event: JournalEvent, context: EventSemanticContext) => {
    ...
    if (event.type === "tool.call.started") {
        context.toolCalls.set(toolCallKey(event.payload.turnId, event.payload.toolCallId), {
    ...
    if (event.type === "tool.call.source") {
        const call = toolCallOf(context, event.payload.turnId, event.payload.toolCallId);
        call.sourceRecorded = true;
    }

    if (event.type === "tool.call.completed" || event.type === "tool.call.failed") {
        const call = context.toolCalls.get(toolCallKey(event.payload.turnId, event.payload.toolCallId));

        if (!call)
            throw new Error(`Tool call ${event.payload.toolCallId} was not validated.`);

        call.status = event.type === "tool.call.completed" ? "completed" : "failed";
    }

    if (event.type === "turn.interrupted") {
        for (const call of runningToolCallsOf(context, event.payload.turnId))
