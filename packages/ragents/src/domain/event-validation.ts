...
const questionOf = (value: unknown, path: string) => {
    if (value === null)
        return;

    const question = exactObject(value, path, ["options", "multi"]);
    stringArrayOf(question.options, `${path}.options`);
    booleanOf(question.multi, `${path}.multi`);
};
...
const payloadOf = (type: EventType, value: unknown, path: string) => {
    switch (type) {
        case "run.created": {
            ...
            const owner = exactObject(payload.owner, `${path}.owner`, ["id", "handle", "displayName", "grants"]);
        ...
        case "action.proposed": {
            ...
            if (payload.kind !== "action" && payload.kind !== "question")
                fail(`${path}.kind`, "must be action or question");

            if (payload.kind === "question")
                questionOf(payload.question, `${path}.question`);
            else if (Object.hasOwn(payload, "question"))
                fail(`${path}.question`, "is only supported for questions");
        ...
        case "action.resolved": {
            const payload = exactObject(value, path, ["actionId", "decision", "response"]);
            stringOf(payload.actionId, `${path}.actionId`);

            if (payload.decision !== "approved" && payload.decision !== "dismissed")
                fail(`${path}.decision`, "must be approved or dismissed");

            nullableStringOf(payload.response, `${path}.response`);
            return;
        }
