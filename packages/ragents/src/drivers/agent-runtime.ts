...
export type AgentThinkingLevel = import("../domain/driver.ts").ThinkingLevel;
...
export type AgentRuntimeDiagnostic = AgentSessionRuntimeDiagnostic & {
...
export type AgentSessionStore = {
    ...
    persisted?: (runId: string, agentId: string, file: string, sessionId: string) => void | Promise<void>;
};
...
const setReplacementSignal = (runtime: AgentSessionRuntime, signal: AbortSignal | undefined): void =>
    (runtime as AgentSessionRuntime & {
        setReplacementSignal(value: AbortSignal | undefined): void;
    }).setReplacementSignal(signal);
...
class TurnDispatcher {
    ...
    initialize() {
        ...
        this.#refreshBaseTools();
        api.setActiveTools(this.#context.allowedToolNames === null ? this.#baseActiveTools : []);

        if (this.#turn)
            this.#configureTools(this.#turn.request);
    }

    needsReload(request: TurnRequest<"agent">) {
        const current = new Set(request.tools.map((tool) => tool.name));

        return request.workspaceTools.some((name) => this.#registeredNames.has(name) && !current.has(name));
    }
    ...
    activate(request: TurnRequest<"agent">, signal: AbortSignal) {
        ...
        this.#turn = { request, signal, usage: emptyUsage(), failure: null };
    }
    ...
    deactivate() {
        ...
        if (this.#api) {
            this.#refreshBaseTools();
            this.#api.setActiveTools(this.#context.allowedToolNames === null ? this.#baseActiveTools : []);
        }
    }

    result(): { failure: string | null; usage: TurnUsage } {
        return this.#turn
            ? { failure: this.#turn.failure, usage: this.#turn.usage }
    ...
    handleEvent(raw: unknown) {
        ...
        if (message.stopReason === "error")
            turn.failure ??= message.errorMessage ?? "The model provider returned an error.";
    ...
    #configureTools(request: TurnRequest<"agent">) {
        ...
        for (const entry of request.tools) {
            ...
            if (existing && existing.sourceInfo.source !== "builtin" && !this.#registeredNames.has(entry.name))
        ...
        this.#applyActiveTools(request);
    }

    #applyActiveTools(request: TurnRequest<"agent">) {
        const api = this.#requiredApi();
        const allowedNames = request.allowedToolNames === null ? null : new Set(request.allowedToolNames);
        const managed = [...this.#activeManagedNames];
        const active = allowedNames === null
            ? [
                ...this.#baseActiveTools.filter((name) => !this.#registeredNames.has(name)),
                ...request.workspaceTools.filter((name) => !this.#activeManagedNames.has(name)),
                ...managed,
            ]
            : [
                ...request.workspaceTools.filter((name) => !this.#registeredNames.has(name)),
                ...this.#baseActiveTools.filter((name) => allowedNames.has(name) && !this.#registeredNames.has(name)),
                ...managed,
            ];
        const available = new Set(api.getAllTools().map((tool) => tool.name));
        const missing = active.filter((name) => !available.has(name));

        if (missing.length > 0)
            throw new Error(`agent tools are not registered: ${missing.join(", ")}.`);

        api.setActiveTools([...new Set(active)]);
    }

    #refreshBaseTools() {
        const api = this.#requiredApi();
        this.#baseActiveTools = api
            .getAllTools()
            .filter((tool) => tool.sourceInfo.source !== "builtin" && !this.#registeredNames.has(tool.name))
            .map((tool) => tool.name);
    }
...
class PersistedAgentSession {
    ...
    constructor(context: AgentRuntimeContext, store: AgentSessionStore | undefined) {
        this.#context = context;
        this.#store = store;
    }
    ...
    async open(): Promise<SessionManager> {
        if (!this.#store)
            return SessionManager.inMemory(this.#context.workspace);
    ...
    async persist(file: string | undefined, sessionId: string) {
        ...
        await this.#store.persisted?.(this.#context.runId, this.#context.agentId, resolvedFile, sessionId);
    }
    ...
    async #newManager() {
        ...
        await syncDirectory(this.#directory);
        const manager = SessionManager.open(file, this.#directory, this.#context.workspace);
        syncFile(file);
    ...
    async #forkedManager(sourceAgentId: string) {
        ...
        const source = SessionManager.open(sourceFile, resolve(sourceDirectory), this.#context.workspace);
        const header = {
            type: "session",
    ...
    async #openStored(record: SessionIdentityRecord) {
        ...
        await access(file);
        const manager = SessionManager.open(file, directory, this.#context.workspace);
        this.#assertWorkspace(manager);
    ...
    #assertWorkspace(manager: SessionManager) {
        if (resolve(manager.getCwd()) !== resolve(this.#context.workspace))
            throw new Error(`agent session workspace ${manager.getCwd()} does not match ${this.#context.workspace}.`);
    }
...
class ManagedAgentRuntime {
    ...
    readonly #defaultThinking: AgentThinkingLevel | undefined;
    readonly #sessionShutdownTimeoutMs: number | undefined;
    ...
    constructor(options: {
        ...
        defaultThinking: AgentThinkingLevel | undefined;
        sessionShutdownTimeoutMs: number | undefined;
        turnAbortTimeoutMs: number;
        ...
        this.#sessionShutdownTimeoutMs = options.sessionShutdownTimeoutMs;
    ...
    async runTurn(request: TurnRequest<"agent">, signal: AbortSignal): Promise<TurnResult> {
        ...
        this.#running = true;
        setReplacementSignal(this.#runtime, turnSignal);
        ...
        try {
            ...
            if (this.#dispatcher.needsReload(request)) {
                const reload = this.#track(this.#runtime.session.reload({
                    signal: turnSignal,
                    ...(this.#sessionShutdownTimeoutMs !== undefined
                        ? { sessionShutdownTimeoutMs: this.#sessionShutdownTimeoutMs }
                        : {}),
                }));
                await awaitWithSignal(reload, turnSignal);
                this.#assertResourceDiagnostics();
            ...
            try {
                ...
                setReplacementSignal(this.#runtime, undefined);
    ...
    #assertResourceDiagnostics() {
        const loader = this.#runtime.services.resourceLoader;
        const diagnostics = [
            ...loader.getSkills().diagnostics.map((entry) => `skill ${entry.path ?? "?"}: ${entry.message}`),
            ...loader.getPrompts().diagnostics.map((entry) => `prompt ${entry.path ?? "?"}: ${entry.message}`),
        ];

        if (diagnostics.length > 0)
            throw new Error(`Das Laden der Ressourcen ist unvollständig: ${diagnostics.join("; ")}`);
    }
}
...
export class AgentRuntimeManager {
    ...
    async #create(context: AgentRuntimeContext, request: TurnRequest<"agent">, signal: AbortSignal) {
        ...
        const identity = new PersistedAgentSession(context, this.#options.sessions);
        ...
        const resolvedFactories = loadHostFactories && this.#options.resolveExtensionFactories
            ...
            : undefined;
        const resolvedSkillPaths = managedEnvironment && this.#options.resolveSkillPaths
            ? await awaitWithSignal(this.#options.resolveSkillPaths(context, signal), signal)
            : undefined;
        const skillPaths = managedEnvironment
            ? [...new Set([...(this.#options.skillPaths ?? []), ...(resolvedSkillPaths ?? [])])]
            : [];
        const skillPreload = managedEnvironment && skillPaths.length > 0 && this.#options.skillPreload !== false
            ? createSkillPreloadExtension({
                modelRuntime,
        ...
        const factory: CreateAgentSessionRuntimeFactory = async (runtimeOptions) => {
            if (resolve(runtimeOptions.cwd) !== context.workspace)
                throw new Error(`agent extension attempted to switch ${context.agentId} to workspace ${runtimeOptions.cwd}.`);
            ...
            this.#assertCreationDiagnostics(context, services.diagnostics);
            const settingsErrors = settingsManager.drainErrors();

            if (settingsErrors.length > 0)
                throw new Error(settingsErrors.map((entry) => `${entry.scope}: ${entry.error.message}`).join("; "));
        ...
        const runtime = await createAgentSessionRuntime(factory, {
            ...
            ...(this.#options.sessionShutdownTimeoutMs !== undefined
                ? { sessionShutdownTimeoutMs: this.#options.sessionShutdownTimeoutMs }
                : {}),
        });
    ...
    #assertCreationDiagnostics(context: AgentRuntimeContext, diagnostics: AgentSessionRuntimeDiagnostic[]) {
        for (const diagnostic of diagnostics)
            this.#options.onDiagnostic?.({ ...diagnostic, context });
