...
import { inheritedGrants } from "../runtime/guards.ts";
import type { Orchestration } from "../runtime/orchestration.ts";
import { descendantsOf, lineageSteps, stopLineage } from "../runtime/stop.ts";
...
const actorOf = (view: RunView, reference: string) => {
    const direct = view.actors.find((entry) => entry.id === reference);

    if (direct)
        return direct;

    const wanted = reference.trim().replace(/^@/, "").toLowerCase();
    const sharing = view.actors.filter((entry) => entry.handle === wanted);
    const active = sharing.find((entry) => entry.kind === "human" || entry.lifecycle.kind !== "stopped");
    const found = active ?? sharing.at(-1);

    if (!found)
        throw new Error(`Actor ${reference} does not exist in this run.`);
...
const handleOf = (view: RunView, actorId: string) => {
...
const subscriptionView = (view: RunView, subscription: EventSubscription) => {
    ...
    return {
        ...
        sources: subscription.sourceActorIds
            ? subscription.sourceActorIds.map((actorId) => handleOf(view, actorId))
            : null,
...
export const agentTools: RunFunction[] = [
    ...
    tool({
        ...
        run: ({ runtime, caller, context }, toolCallId, input) => {
            ...
            const sourceActorIds = input.sourceActorIds
                ? [...new Set(input.sourceActorIds.map((reference) => actorOf(view, reference).id))]
                : null;
    ...
    tool({
        ...
        run: ({ runtime, caller }, _toolCallId, input) => {
            ...
            const actorIds = input.actorIds
                ? new Set(input.actorIds.map((reference) => actorOf(view, reference).id))
                : null;
    ...
    tool({
        ...
        run: async ({ runtime, caller, catalog, context, eventsFor, resolveToolsFor }, toolCallId, input) => {
            ...
            const actor = actorOf(runtime.view(caller.runId), caller.actorId);
    ...
    tool({
        ...
        run: ({ runtime, caller, context, eventsFor }, toolCallId, input) => {
            ...
            if (input.primaryActor !== undefined) {
                const target = actorOf(runtime.view(caller.runId), input.primaryActor);
    ...
    tool({
        ...
        run: ({ runtime, caller, context, eventsFor }, toolCallId, input) => {
            const view = runtime.view(caller.runId);
            const target = actorOf(view, input.actorId);
    ...
    tool({
        ...
        run: ({ runtime, caller, context, eventsFor }, toolCallId, input) => {
            ...
            const after = runtime.stopActor(context(toolCallId, "stop"), caller.runId, target.id, input.reason);
            const lineage = descendantsOf(after, target.id);
            const failures = stopLineage(runtime, caller.runId, lineage, input.reason, (step) => ({
                ...context(toolCallId, step),
                actorId: after.ownerId,
            }));

            if (failures.length > 0)
                throw new AggregateError(failures, `${target.handle} konnte nicht vollständig gestoppt werden.`);

            return ["stop", ...lineageSteps(lineage)].flatMap((step) => eventsFor(toolCallId, step));
        },
    }),
    tool({
