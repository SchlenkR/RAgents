...
const namedExtension = (extension: InlineExtension): string | undefined =>
  typeof extension === "function" ? undefined : extension.name;

const extensionFactoryName = (extension: InlineExtension): string | null => {
  if (typeof extension !== "function") return extension.name;
...
export class AgentContributionRegistry {
  ...
  register(owner: string, contributions: readonly AgentContribution[]): void {
    ...
    for (const contribution of contributions) {
      if (contribution.tools.length > 0 && (contribution.global?.length ?? 0) === 0 && !contribution.resolve) {
        throw new Error(`Agent-Beitrag ${contribution.id} deklariert Werkzeuge, aber keine Extension-Factory`);
      }
      for (const tool of contribution.tools) {
        if (!tool.name.trim() || !tool.description.trim() || !tool.availabilityDetail.trim()) {
          throw new Error(`Agent-Beitrag ${contribution.id} hat einen unvollständigen Werkzeug-Descriptor`);
        }
        if (names.has(tool.name)) {
          throw new Error(`Agent-Werkzeug ${tool.name} ist innerhalb der Registrierung mehrfach deklariert`);
        }
        const existing = this.#toolOwners.get(tool.name);
        if (existing) throw new Error(`Agent-Werkzeug ${tool.name} wird bereits von ${existing} deklariert`);
        names.add(tool.name);
      }
    }
    this.#contributions.register(owner, contributions);
    for (const name of names) this.#toolOwners.set(name, owner);
  }

  global(): readonly InlineExtension[] {
    return this.#unique(this.#contributions.entries().flatMap(({ value }) => value.global ?? []));
  }

  async resolve(context: AgentContributionContext): Promise<readonly InlineExtension[]> {
    const contextual = await Promise.all(this.#contributions.entries().map(async ({ value }) =>
      value.resolve ? await value.resolve(context) : []));
    return this.#unique([...this.global(), ...contextual.flat()]);
  }
  ...
  describe(): readonly PublicAgentExtensionContribution[] {
    return this.#contributions.entries().map(({ owner, value }) => ({
      ...
      factories: (value.global ?? []).map((extension) => ({
        name: extensionFactoryName(extension),
        scope: "global" as const,
      })),
      resolvesPerAgent: Boolean(value.resolve),
    }));
  }

  describeTools(): readonly PublicToolDescriptor[] {
    return this.#contributions.entries().flatMap(({ owner, value }) =>
      value.tools.map((tool) => ({
        id: `agent-extension:${owner}:${value.id}:${tool.name}`,
        owner,
        source: value.id,
        kind: "agent-extension" as const,
        ...tool,
        nativeTool: true,
      })));
  }
  ...
  tools(): readonly AgentToolDescriptor[] {
    return this.#contributions.entries().flatMap(({ value }) => value.tools);
  }

  #unique(extensions: readonly InlineExtension[]): readonly InlineExtension[] {
    const names = new Set<string>();
    for (const extension of extensions) {
      const name = namedExtension(extension);
      if (!name) continue;
      if (names.has(name)) throw new Error(`Agent-Extension ${name} ist mehrfach registriert`);
      names.add(name);
    }
    return extensions;
  }
...
const validateStartEntry = (entry: StartEntryContribution): void => {
  const base = ["id", "title", "description", "order", "guide", "tags", "action"];
  ...
  switch (entry.action) {
    case "skill":
      ...
      return;
    case "script":
      assertKnownFields(entry, entry, [...base, "script"], "Einstieg");
...
export class StartEntryContributionRegistry {
    ...
    return this.#entries
      ...
      .map(({ owner, value }): PublicStartEntry => {
        ...
        switch (value.action) {
          ...
          case "script":
            return { ...base, action: "script", coordinator: value.script.coordinator };
        }
      })
      .sort(byOrderThenId);
  }
  ...
  scriptPackage(entryId: string): (RunScriptPackage & { entry: PublicStartEntry }) | undefined {
    ...
    const entry = this.describe().find((candidate) => candidate.id === entryId);
...
export class SessionMetadataContributionRegistry {
  ...
  async describe(runId: string): Promise<Readonly<Record<string, unknown>>> {
    const entries = await Promise.all(this.#metadata.entries().map(async ({ value }) => [
      value.id,
      await value.describe({ runId }),
    ] as const));
...
export class PluginHost {
  ...
  publicProfile(access: AccessContext = unrestrictedAccess): PublicPluginProfile {
    ...
    return {
      ...
      startEntries: this.startEntries.describe().filter((entry) => canStartEntry(access, entry.id)
