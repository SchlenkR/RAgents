...
const assertRunConfigurator = (state: RunState, caller: Actor) => {
    if (caller.id === state.ownerId) return;
    if (!hasCapability(caller, "run.configure", { kind: "run" }))
...
export const retitleRun = (title: string): Decision => (state, context) => {
    assertRunConfigurator(state, actorOf(state, context));
...
const forkSourceOf = (state: RunState, reference: string): string => {
    const source = addressedActorOf(state, reference);
...
export const restartActor =
    ...
    (state, context) => {
        ...
        const collision = [...state.actors.values()].find((entry) =>
            entry.id !== target.id && entry.handle === target.handle && isActiveActor(entry));

        if (collision)
            throw new DomainError(
                "handle-exists",
                `Handle @${target.handle} is now taken by ${collision.id}. Stop that actor first or leave this one stopped.`,
            );

        return [event(context, { type: "actor.restarted", payload: { actorId: target.id, reason: clean(reason, "reason") } })];
    };
    ...
    (state, context) => {
        ...
        return [
            ...(target.lifecycle.kind === "running"
                ...
                : []),
            event(context, { type: "actor.stopped", payload: { actorId, reason: cleanReason } }),
            ...subscriptions.map((subscription) => event(context, {
