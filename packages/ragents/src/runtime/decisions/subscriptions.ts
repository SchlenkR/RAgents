...
import { isPendingActorInput, type EventSubscription, type ObservableEventType, type RunState } from "../../domain/model.ts";
...
import { actorById, actorOf, assertCapability, clean, executableActorOf } from "../guards.ts";
...
export const matchesSubscription = (state: RunState, subscription: EventSubscription, source: JournalEvent) => {
    ...
    if (subscription.sourceActorKinds) {
        const sourceActor = state.actors.get(source.actorId);
