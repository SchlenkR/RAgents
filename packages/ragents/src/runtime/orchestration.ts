...
export class Orchestration {
    ...
    assertActorTurn(runId: string, actorId: string, turnId: string) {
        actorOf(this.state(runId), { actorId, turnId });
    }
    ...
    listSubscriptions(runId: string, subscriberId?: string) {
        const state = this.state(runId);
        const resolved = subscriberId ? addressedActorOf(state, subscriberId).id : null;
    ...
    resolveAction(
        ...
        runId: string,
        actionId: string,
        input: { decision: Exclude<ActionStatus, "pending">; response?: string | null },
    ) {
    ...
    publishArtifact(
        ...
        assertCapability(actorOf(this.state(runId), context), "artifact.publish", { kind: "run" });
    ...
    artifactContent(runId: string, artifactId: string, actorId: string) {
        ...
        assertArtifactRead(state, addressedActorOf(state, actorId), artifact);
    ...
    #dispatchCreatorAlerts(sources: readonly JournalEvent[]) {
        for (const source of sources) {
            ...
            const alert = source.type === "turn.finished" && source.payload.outcome === "failed"
                ? { kind: "failed" as const, reason: source.payload.reason }
                : source.type === "turn.interrupted"
                    ? { kind: "interrupted" as const, reason: source.payload.reason }
                    : null;
            ...
            const state = this.state(source.runId);
            const child = state.actors.get(source.actorId);

            if (!child || child.kind === "human" || !child.createdBy || child.createdBy === child.id)
                continue;
