...
const stopEventsOf = (runtime: Orchestration, runId: string) =>
    runtime.events(runId).filter((entry) => entry.type === "actor.stopped" || entry.type === "turn.interrupted");

test("actor_stop stops the target and its active descendants in one command in the name of the caller", async () => {
    const { journal, runtime, view } = fixture();
    const branchGrants = [grant("agent.spawn"), grant("execution.stopOwned"), grant("actor.input")];

    try {
        const spawned = runtime.spawnAgent({ actorId: view.ownerId, commandId: "spawn-lead" }, view.id, {
            handle: "lead", displayName: "lead", prompt: "Lead.", execution: manualExecution(), grants: branchGrants, toolNames: null,
        });
        const lead = spawned.actors.find((entry) => entry.handle === "lead");
        assert.ok(lead);
        const queued = runtime.enqueueInput({ actorId: view.ownerId, commandId: "input-lead" }, view.id, { actorId: lead.id, content: "Lead." });
        ...
        assert.deepEqual(
            stops.map((entry) => [entry.type, entry.type === "actor.stopped" ? entry.payload.actorId : entry.payload.turnId]),
            [["turn.interrupted", reviewerTurn], ["actor.stopped", reviewer.id], ["actor.stopped", helper.id]],
        );
        assert.equal(new Set(stops.map((entry) => entry.commandId)).size, 1);
        assert.ok(stops.every((entry) => entry.actorId === lead.id));
    } finally {
        journal.close();
    }
});

test("a run stop writes the stops of all actors but the primary one in one command", async () => {
    const { journal, runtime, view } = fixture();

    try {
        const primary = spawn(runtime, view, "primary", view.ownerId, undefined, [grant("agent.spawn")]);
        const primaryTurn = start(runtime, runtime.view(view.id), primary.id, "primary");
        const first = spawn(runtime, runtime.view(view.id), "first", primary.id, primaryTurn);
        const second = spawn(runtime, runtime.view(view.id), "second");
        start(runtime, runtime.view(view.id), first.id, "first");
        start(runtime, runtime.view(view.id), second.id, "second");
        ...
        const stops = stopEventsOf(runtime, view.id);
        assert.deepEqual(
            stops.filter((entry) => entry.type === "actor.stopped").map((entry) => entry.type === "actor.stopped" ? entry.payload.actorId : "").sort(),
            [first.id, second.id].sort(),
        );
        assert.equal(new Set(stops.map((entry) => entry.commandId)).size, 1);
        assert.ok(stops.every((entry) => entry.actorId === view.ownerId));
        assert.deepEqual(
            runtime.view(view.id).inputs.filter((entry) => entry.actorId === primary.id && entry.lifecycle.kind === "pending"),
            [],
        );
    } finally {
        journal.close();
...
test("a repeated run stop with the same command ID also stops an actor created in between", async () => {
    const { journal, runtime, view } = fixture();
