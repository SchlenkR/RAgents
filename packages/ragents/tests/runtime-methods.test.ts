...
import type { TurnInterruption } from "../src/agents/scheduler.ts";
...
const fixture = (overrides: Partial<Pick<RuntimeMethodOptions, "abortTurns" | "assertAvailable" | "assertRunUsable" | "assertRunRights" | "stopRun">> = {}) => {
  const services = testServices();
  const journal = new Journal(":memory:", services);
  const runtime = new Orchestration(journal, services);
  const rights: string[] = [];
  const interruptions: Array<{ runId: string; actorId: string; interruption: TurnInterruption }> = [];
...
test("interrupting a turn needs the stop right, resolves handles and names the owner", async () => {
  const f = fixture();
  try {
...
test("a run stop that is still running does not hold up the writes of another run", async () => {
  const f = fixture({ stopRun: () => new Promise(() => undefined) });
  try {
    const stopping = f.createRun("Stopping");
    const other = f.spawn(f.createRun("Other").id, "worker");
    const worker = other.actors.find((entry) => entry.handle === "worker")!;
...
test("stopping an actor over the method stops its whole branch in one command", async () => {
  const f = fixture();
  try {
    let run = f.createRun("Branch");
    run = f.spawn(run.id, "parent", [{ capability: "agent.spawn", scope: { kind: "run" }, delegable: true }]);
    const parent = run.actors.find((entry) => entry.handle === "parent")!;
    run = f.runtime.enqueueInput({ actorId: run.ownerId, commandId: "parent-input" }, run.id, { actorId: parent.id, content: "Create a child." });
    run = f.runtime.startTurn({ actorId: parent.id, commandId: "parent-turn" }, run.id, parent.id, run.inputs.at(-1)!.id);
    const turn = run.turns.at(-1)!;
    run = f.runtime.spawnAgent({ actorId: parent.id, commandId: "spawn-child", turnId: turn.id }, run.id, { handle: "child", displayName: "Child", prompt: "", execution: manualExecution(), grants: [], toolNames: [] });
    await f.method(runContracts.stopActor)({ runId: run.id, commandId: "stop-parent", actorId: "@parent", reason: "Done." }, context());
    const stops = f.runtime.events(run.id).filter((event) => event.type === "actor.stopped" || event.type === "turn.interrupted");
    assert.deepEqual(stops.map((event) => event.type), ["turn.interrupted", "actor.stopped", "actor.stopped"]);
    assert.deepEqual([...new Set(stops.map((event) => event.commandId))], ["stop-parent"]);
    assert.ok(stops.every((event) => event.actorId === run.ownerId));
  } finally {
    f.close();
  }
});
