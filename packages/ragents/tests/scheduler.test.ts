...
for (const stopActor of [false, true]) {
    test(`interrupted driver output ${stopActor ? "cannot append after actor stop" : "precedes the turn interruption"}`, async () => {
        const setup = setupRun();
        const started = deferred();
        const driver = new FakeDriver(async (request, signal) => {
            request.publish({ kind: "text", delta: "Visible unfinished answer" });
            started.resolve();
            await new Promise<void>((resolve) => signal.addEventListener("abort", () => resolve(), { once: true }));
            request.emit({ kind: "assistant-interrupted", text: "Visible unfinished answer" });
            return { failure: null, usage: noUsage() };
        });
        const scheduler = new TurnScheduler(setup.runtime, setup.journal, { drivers: registryOf(driver), catalog });
        try {
            postTo(setup.runtime, setup.view, setup.agent.id, "partial-input", "Long work");
            scheduler.start();
            await started.promise;
            if (stopActor) setup.runtime.stopActor(
                { actorId: setup.view.ownerId, commandId: "stop-worker" }, setup.view.id, setup.agent.id, "Stopped",
            );
            else scheduler.abort(setup.view.id, setup.agent.id);
            await scheduler.waitForIdle();
            const events = setup.runtime.events(setup.view.id);
            assert.deepEqual(events.filter((event) => event.type === "model.output.interrupted" || event.type === "turn.interrupted")
                .map((event) => event.type), stopActor ? ["turn.interrupted"] : ["model.output.interrupted", "turn.interrupted"]);
            assert.equal(events.some((event) => event.type === "model.output.completed"), false);
            assert.deepEqual(setup.runtime.view(setup.view.id).turns[0]?.outputs, []);
            assert.equal(driver.requests.length, 1);
        } finally {
            await scheduler.stop();
            setup.journal.close();
...
for (const withWorkspace of [true, false]) {
    test(`the workspace description reaches an actor ${withWorkspace ? "whose workspace tools come from a function" : "without workspace tools"}`, async () => {
        const setup = withWorkspace
            ? setupRun({
                grants: [{ capability: "workspace.use", scope: { kind: "run" }, delegable: true }],
                toolNames: ["read"],
            })
            : setupRun({ toolNames: null });
        ...
        const registry = new ToolRegistry().register({ name: "host-workspace", dynamic: true, descriptors: [], tools: () => [read] });
        const driver = new FakeDriver(async (request) => {
            assert.equal(/# Working directory/.test(request.systemPrompt), withWorkspace);
            if (withWorkspace) {
                assert.ok(request.refreshTools);
                const refreshed = await request.refreshTools();
                assert.match(refreshed.systemPrompt, /# Working directory/);
            }
            return { failure: null, usage: noUsage() };
        });
        const scheduler = new TurnScheduler(setup.runtime, setup.journal, {
            drivers: registryOf(driver),
            catalog,
            ...(withWorkspace ? { registry } : {}),
            workspaceToolNaming: { agentToolNames: () => ({ read: ["read"], write: [], execute: [] }) },
            workspaces: {
                ensure: () => process.cwd(),
        ...
        try {
            postTo(setup.runtime, setup.view, setup.agent.id, "workspace-input", "Where are you?");
            scheduler.start();
            await scheduler.waitForIdle();
...
const toolCallEnds = async (outcome: "abort" | "failure") => {
    const setup = setupRun();
    const started = deferred();
    const driver = new FakeDriver(async (request, signal) => {
        request.recordTool?.({ kind: "started", id: "open-call", name: "expected", input: {} });
        started.resolve();
        if (outcome === "abort")
            await new Promise<void>((resolve) => signal.addEventListener("abort", () => resolve(), { once: true }));
    ...
    try {
        postTo(setup.runtime, setup.view, setup.agent.id, "open-call-input", "Use the tool.");
        scheduler.start();
        await started.promise;
        if (outcome === "abort")
            await scheduler.stop();
        ...
        return {
            ...
            callStatus: turn?.toolCalls.map((call) => call.status),
            events: setup.runtime.events(setup.view.id).filter((event) => event.type.startsWith("tool.call.")).map((event) => event.type),
        };
    } finally {
        await scheduler.stop();
        setup.journal.close();
...
test("an aborted turn ends its open tool calls as interrupted, from the turn end alone", async () => {
    assert.deepEqual(await toolCallEnds("abort"), {
        turnStatus: "interrupted",
        callStatus: ["interrupted"],
        events: ["tool.call.started"],
    });
});

test("a failed turn ends its open tool calls as interrupted, from the turn end alone", async () => {
    assert.deepEqual(await toolCallEnds("failure"), {
        turnStatus: "failed",
        callStatus: ["interrupted"],
        events: ["tool.call.started"],
    });
...
test("an actor that requests tools no host function resolves stops at creation with the cause", async () => {
    const setup = setupRun();
    const known = (name: string, available: boolean) => defineRunFunction({
        name,
        label: name,
    ...
    const registry = new ToolRegistry().register({
        ...
        dynamic: true,
        descriptors: [],
        tools: () => [known("known_read", true), known("known_locked", false)],
    });
    const driver = new FakeDriver(async () => ({ failure: null, usage: noUsage() }));
    const scheduler = new TurnScheduler(setup.runtime, setup.journal, { drivers: registryOf(driver), catalog, registry, onError: () => {} });

    try {
        scheduler.start();
        const spawn = (handle: string, toolNames: readonly string[]) => {
            const view = setup.runtime.spawnAgent({ actorId: setup.view.ownerId, commandId: `spawn-${handle}` }, setup.view.id, {
                handle,
                displayName: handle,
            ...
            assert.ok(actor);
            postTo(setup.runtime, setup.view, actor.id, `input-${handle}`, "Do exactly this.");
            return actor.id;
        };
        const typo = spawn("typo", ["known_read", "known_raed"]);
        const locked = spawn("locked", ["known_read", "known_locked"]);
        await scheduler.waitForIdle();

        const actors = setup.runtime.view(setup.view.id).actors.filter((entry) => entry.kind !== "human");
        const stopped = actors.find((entry) => entry.id === typo);
        assert.equal(stopped?.lifecycle.kind, "stopped");
        const stop = setup.runtime.events(setup.view.id).find((entry) => entry.type === "actor.stopped" && entry.payload.actorId === typo);
