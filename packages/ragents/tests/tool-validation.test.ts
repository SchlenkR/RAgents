...
import { fauxAssistantMessage, fauxToolCall, registerFauxProvider, type Context } from "@aicontainer/ai";
...
const registryWithTools = () => {
    ...
    registry.register({
        ...
        descriptors: [strictTool, contextTool, nestedTool].map((tool) => ({
            name: tool.name,
        ...
        tools: () => [strictTool, contextTool, nestedTool],
    });
