...
test("interrupted model output replays as history without an output result or subscription delivery", () => {
    const { journal, runtime, view } = create();
    try {
        const source = spawn(runtime, view, "source").actor;
        const observer = spawn(runtime, runtime.view(view.id), "observer").actor;
        runtime.createSubscription({ actorId: view.ownerId, commandId: "observe-output" }, view.id, {
            subscriberId: observer.id,
            sourceActorIds: [source.id],
            eventTypes: ["model.output.completed"],
        });
        const { turnId } = start(runtime, runtime.view(view.id), source.id, "partial-answer");
        const context = { actorId: source.id, commandId: "partial-output", turnId };
        runtime.appendInterruptedModelOutput(context, view.id, source.id, { turnId, text: "Visible unfinished answer" });
        runtime.appendInterruptedModelOutput(context, view.id, source.id, { turnId, text: "Visible unfinished answer" });
        assert.equal(runtime.events(view.id).filter((event) => event.type === "model.output.interrupted").length, 1);
        assert.deepEqual(runtime.view(view.id).turns[0]?.outputs, []);
        assert.equal(runtime.view(view.id).inputs.some((input) => input.actorId === observer.id), false);
        assert.throws(() => runtime.createSubscription({ actorId: view.ownerId, commandId: "observe-partial" }, view.id, {
            subscriberId: observer.id,
            eventTypes: ["model.output.interrupted" as never],
        }), /model\.output\.interrupted\. Subscribable event types:/);
        runtime.interruptTurn({ actorId: source.id, commandId: "interrupt", turnId }, view.id, source.id, { turnId, reason: "Stopped" });
        assert.deepEqual(viewOf(project(runtime.events(view.id))!), runtime.view(view.id));
        assert.throws(() => runtime.appendInterruptedModelOutput(
            { ...context, commandId: "late-output" }, view.id, source.id, { turnId, text: "Too late" },
        ), /is not running/);
    } finally {
        journal.close();
    }
...
test("action resolution stays an event and does not route actor input", () => {
    const { journal, runtime, view } = create();
    const worker = spawn(runtime, view, "worker").actor;
    const turn = start(runtime, runtime.view(view.id), worker.id, "ask");
    const proposed = runtime.proposeAction(
        { actorId: worker.id, commandId: "ask", turnId: turn.turnId },
        view.id,
        { owner: "ragents.ask", title: "Choose", payload: { options: ["A", "B"] } },
    );
...
test("a run keeps the user it was created for, and a run created without a login has none", () => {
    const services = testServices();
    const journal = new Journal(":memory:", services);
    const runtime = new Orchestration(journal, services);
    const owned = runtime.createRun(
        { commandId: "create-owned" },
        { title: "Owned", ownerHandle: "alice", ownerDisplayName: "Alice", ownerUserId: "alice" },
    );
    const open = runtime.createRun(
        { commandId: "create-open" },
        { title: "Open", ownerHandle: "owner", ownerDisplayName: "Owner" },
    );

    assert.equal(runtime.state(owned.id).ownerUserId, "alice");
    assert.equal(runtime.state(open.id).ownerUserId, null);
    assert.equal("ownerUserId" in owned, false);
    assert.equal("ownerUserId" in runtime.state(owned.id).actors.get(owned.ownerId)!, false);

    const created = runtime.events(owned.id)[0]!;
    assert.equal(created.type === "run.created" && created.payload.owner.userId, "alice");
    assert.equal(project(runtime.events(owned.id))?.ownerUserId, "alice");
    assert.equal(project(runtime.events(open.id))?.ownerUserId, null);
    journal.close();
});
