...
function isEmptyResponse(message: AssistantMessage): boolean {
	return (
		...
		!message.content.some((c) => c.type === "toolCall" || (c.type === "text" && c.text.trim().length > 0))
	);
}

function failEmptyResponse(message: AssistantMessage, failOnEmpty: boolean): AssistantMessage {
	return failOnEmpty && isEmptyResponse(message)
		? { ...message, stopReason: "error", errorMessage: EMPTY_RESPONSE_FAILURE }
		: message;
}
...
/** The loop ends like an aborted stream, but no request reaches the model. */
function abortedBeforeRequest(config: AgentLoopConfig): AssistantMessage {
	return {
		role: "assistant",
		content: [],
		...
		stopReason: "aborted",
		errorMessage: "Request was aborted before it reached the model",
		timestamp: Date.now(),
	};
}
...
async function streamAssistantResponse(
	...
	if (signal?.aborted) {
		const aborted = abortedBeforeRequest(config);
		context.messages.push(aborted);
		await emit({ type: "message_start", message: { ...aborted } });
		await emit({ type: "message_end", message: aborted });
		return aborted;
	}
	...
	for await (const event of response) {
		switch (event.type) {
			...
			case "done":
			case "error": {
				const finalMessage = failEmptyResponse(await response.result(), failOnEmpty);
				if (addedPartial) {
...
async function prepareToolCall(
	...
	try {
		...
		if (!tool) {
			return {
				kind: "immediate",
				result: createErrorToolResult(config.formatUnknownToolError?.(toolCall.name) ?? `Tool ${toolCall.name} not found`),
				isError: true,
			};
		}
