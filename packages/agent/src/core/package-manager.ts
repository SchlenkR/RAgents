...
type SkillDiscoveryMode = "native" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "native" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readAgentManifestFile(packageJsonPath: string): AgentManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { agent?: AgentManifest };
		return pkg.agent ?? null;
	} catch {
		return null;
	}
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "native");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private async updateConfiguredSources(sources: ConfiguredUpdateSource[]): Promise<void> {
		...
		for (const entry of npmCandidates) {
			if (entry.scope === "user") {
				userNpmUpdates.push(entry);
			} else {
				projectNpmUpdates.push(entry);
			}
		}
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Erweiterungen laufen im Agenten und lösen dessen APIs über die Loader-Aliase auf.
		// Peer-Auflösung bei verwalteten Installationen abschalten, damit Paketmanager keine
		// vom Host bereitgestellten Peers nachinstallieren.
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from the project config dir
			addResources(
				"extensions",
			...
			// Project skills from the project config dir
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "native"),
				projectMetadata,
		...
		// User extensions from the user config dir
		addResources(
			"extensions",
		...
		// User skills from the user config dir
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "native"),
			userMetadata,
