...
export class AgentSession {
	...
	private readonly _extensionSettlements = new Set<Promise<void>>();
	...
	waitForSettlement(): Promise<void> {
		const resourceSettlement = this._resourceLoader.waitForSettlement?.();
		return Promise.allSettled([
			...this._extensionSettlements,
			...(resourceSettlement ? [resourceSettlement] : []),
		]).then(() => undefined);
	}

	private _trackExtensionSettlement(settlement: Promise<void>): void {
		this._extensionSettlements.add(settlement);
		void settlement.then(
			() => this._extensionSettlements.delete(settlement),
			() => this._extensionSettlements.delete(settlement),
		);
	}
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			if (currentAttachments) userContent.push(...currentAttachments);
			messages.push({
				role: "user",
				content: userContent,
				timestamp: Date.now(),
	 ...
	 * @param attachments Optional media attachments to include with the message
	 ...
	 */
	private async _queueSteer(text: string, attachments?: UserAttachment[]): Promise<void> {
	 ...
	 */
	private async _queueFollowUp(text: string, attachments?: UserAttachment[]): Promise<void> {
	...
	async sendUserMessage(
		...
		// Normalize content to text and media attachments.
		let text: string;
		let attachments: UserAttachment[] | undefined;
	...
	async reload(options?: {
		...
		try {
			const shutdown = beginSessionShutdownEvent(
				...
				{ type: "session_shutdown", reason: "reload" },
				{
					...
					...(options?.sessionShutdownTimeoutMs !== undefined
						? { timeoutMs: options.sessionShutdownTimeoutMs }
						: {}),
				},
			);
			this._trackExtensionSettlement(shutdown.settled);
		...
		resetApiProviders();
		await this._resourceLoader.reload(options?.signal ? { signal: options.signal } : undefined);
		if (options?.signal?.aborted) {
			this._resourceLoader.getExtensions().runtime.invalidate("Extension creation was aborted.");
			options.signal.throwIfAborted();
		}
		this._buildRuntime({
