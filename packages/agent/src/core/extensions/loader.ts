...
import * as _bundledAgentCore from "@aicontainer/agent-core";
import * as _bundledAi from "@aicontainer/ai";
import * as _bundledAiOauth from "@aicontainer/ai/oauth";
import * as _bundledAiProviders from "@aicontainer/ai/providers/all";
...
// avoiding a circular dependency. Extensions can import from @aicontainer/agent.
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@aicontainer/agent-core": _bundledAgentCore,
	"@aicontainer/ai": _bundledAi,
	"@aicontainer/ai/oauth": _bundledAiOauth,
	"@aicontainer/ai/providers/all": _bundledAiProviders,
	"@aicontainer/agent": _bundledCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const agentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@aicontainer/agent-core");
	const aiEntry = resolveWorkspaceOrImport("ai/dist/index.js", "@aicontainer/ai");
	const aiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@aicontainer/ai/oauth");
	const aiProvidersEntry = resolveWorkspaceOrImport("ai/dist/providers/all.js", "@aicontainer/ai/providers/all");

	_aliases = {
		"@aicontainer/agent": codingAgentEntry,
		"@aicontainer/agent-core": agentCoreEntry,
		"@aicontainer/ai/providers/all": aiProvidersEntry,
		"@aicontainer/ai/oauth": aiOauthEntry,
		"@aicontainer/ai": aiEntry,
...
async function loadExtension(
	...
	try {
		const factory = await awaitWithSignal(loadExtensionModule(resolvedPath, cacheToken), signal);
...
async function loadExtensionsInternal(
	...
	if (signal?.aborted) {
		resolvedRuntime.invalidate("Extension creation was aborted.");
		throw abortReason(signal);
	}
...
export async function loadExtensions(
	...
	return loadExtensionsInternal(paths, cwd, eventBus, runtime, false, signal, onFactorySettlement);
}
...
export async function loadExtensionsCached(
	...
	return loadExtensionsInternal(paths, cwd, eventBus, runtime, true, signal, onFactorySettlement);
}
...
function readAgentManifest(packageJsonPath: string): AgentManifest | null {
	try {
		...
		if (pkg.agent && typeof pkg.agent === "object") {
			return pkg.agent as AgentManifest;
		}
		return null;
	} catch {
		return null;
