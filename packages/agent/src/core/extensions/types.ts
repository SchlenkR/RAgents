...
export interface ExtensionContext {
	...
	mode: ExtensionMode;
	/** Whether dialog-capable UI is available (true in RPC mode) */
	hasUI: boolean;
	...
	/** Gracefully shutdown the agent and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what the agent loaded without re-discovering resources. */
...
export type InputEventResult =
	| { action: "continue" }
	| { action: "transform"; text: string; images?: ImageContent[]; attachments?: UserAttachment[] }
	| { action: "handled" };
...
export type ExtensionFactory = (pi: ExtensionAPI, context: ExtensionFactoryContext) => void | Promise<void>;
