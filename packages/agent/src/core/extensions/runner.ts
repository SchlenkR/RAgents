...
import type { AgentMessage } from "@aicontainer/agent-core";
import type { ImageContent, Model, ProviderHeaders, UserAttachment } from "@aicontainer/ai";
...
export interface SessionShutdownEventOperation {
	emitted: boolean;
	bounded: Promise<void>;
	settled: Promise<void>;
}

export function beginSessionShutdownEvent(
	extensionRunner: ExtensionRunner,
	event: SessionShutdownEvent,
	options?: { signal?: AbortSignal; timeoutMs?: number },
): SessionShutdownEventOperation {
	...
	const abortReason = () => signal?.reason instanceof Error
		? signal.reason
		: new Error("Session shutdown was aborted.");
	if (signal?.aborted) {
		throw abortReason();
	}

	const settled = extensionRunner.emit(event).then(() => undefined);
	void settled.catch(() => undefined);
	const bounded = (async () => {
		let timer: ReturnType<typeof setTimeout> | undefined;
		let abort: (() => void) | undefined;
		const limit = new Promise<void>((resolve, reject) => {
	...
	return { emitted: true, bounded, settled };
}
...
export async function emitSessionShutdownEvent(
	...
	const operation = beginSessionShutdownEvent(extensionRunner, event, options);
	await operation.bounded;

	return operation.emitted;
}
...
export class ExtensionRunner {
	...
	async emitInput(
		...
		return currentText !== text || currentImages !== images || currentAttachments !== attachments
			? { action: "transform", text: currentText, images: currentImages, attachments: currentAttachments }
			: { action: "continue" };
	}
}
