...
function validateInputModalities<TApi extends Api>(messages: Message[], model: Model<TApi>): void {
	for (const message of messages) {
		...
		for (const block of message.content) {
			if (block.type !== "text" && !model.input.includes(block.type)) {
				throw new Error(`Model ${model.id} does not support ${block.type} input. Supported inputs: ${model.input.join(", ")}.`);
			}
		}
	}
}
...
export function transformMessages<TApi extends Api>(
	...
	// First pass: transform thinking blocks and tool call IDs.
	const transformed = normalizedMessages.map((msg) => {
