import type { OpenRouterOptions } from "./api/ai-sdk.ts";
...
export interface ApiOptionsMap {
	"openai-completions": OpenRouterOptions;
}
...
export type UserAttachment = ImageContent | VideoContent | FileContent;
export type UserContent = TextContent | UserAttachment;
...
export interface OpenAICompletionsCompat {
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for host-controlled thinking values. */
