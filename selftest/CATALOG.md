...
## T13 Hallo Welt auf dem Canvas

Idee: Ein winziger Wunsch nach einer App in der Werkstatt endet als kleine statische
Actor-View auf dem Canvas mit einem kurzen Erstellen-Aktivieren-Ablauf.

Prompt A: "Ich möchte auf der Arbeitsfläche eine kleine App haben, die nur den Text
Hallo Welt zeigt. Bau das bitte so, dass ich sie direkt auf dem Canvas sehe."
...
danach steht die App auf dem Canvas und actor_program_list bestätigt die Installation.
...
Der Host bedient Größenänderung, Einklappen und lokale Vollansicht. Abschluss in unter 5 Minuten.
...
## T14 Gesprächskreis auf der Fläche

Idee: Der Koordinator soll die Fläche neben dem Chat selbst gestalten, bevor die Arbeit
läuft: Gesprächspartner als Kreis, das Thema als Raute, Pfeile in Gesprächsrichtung.
...
losgeht, möchte ich auf der Fläche neben dem Chat sehen, wie das aufgebaut ist: die drei im
Kreis, das Thema als Raute darüber und Pfeile, wer an wen weitergibt. Am Ende eine kurze
Zusammenfassung."

Prompt B: "Drei KIs unterhalten sich im Kreis über ein Thema. Zeig mir den Aufbau auf der
Fläche, dann lass sie reden."

Erwartung: agent_spawn für drei Agenten mit tools: [], danach genau ein
canvas_layout_replace mit einer Gruppe circle (drei Mitglieder als @handle), einer Raute mit
Text, die auch als Knoten platziert ist, und drei Pfeilen (arrow end) zwischen den Handles.
Kein Layout-Fehler im Journal, kein zweiter Aufruf während des Gesprächs, kein Versuch, den
Menschen zu platzieren. Das Gespräch läuft danach wie in T01.

Warum gut: Erster Ende-zu-Ende-Fall für das Layout-Werkzeug: prüft, ob das Modell
Entitäten als @handle schreibt, Formen definiert UND platziert, Linien nur zwischen
Entitäten zieht und die Fläche vor der Arbeit einrichtet statt danach.

## T15 Baum, Raster und der Rest

Idee: Ein Layout mit Baum aus der Abstammung eines Unteragenten, Raster aus Formen und einem
absichtlich nicht platzierten Actor; dazu der Umgang mit einem Werkzeugfehler.

Prompt A: "Ich möchte einen Teamleiter, der sich drei Helfer holt und jedem ein Thema gibt;
jeder Helfer liefert drei Stichpunkte. Dazu einen Beobachter, der am Ende alles kurz
bewertet. Auf der Fläche will ich das Team als Baum sehen, daneben vier Themen als Kreise in
einem Raster mit zwei Spalten, mit gestrichelten Linien von jedem Helfer zu seinem Thema; das
vierte Thema bleibt frei. Den Beobachter ordne bitte nicht selbst ein."

Prompt B: "Teamleiter mit drei Helfern als Baum auf der Fläche, daneben Themen als Kreise
im Raster. Den Beobachter lass weg."

Erwartung: Die Helfer werden vom Teamleiter erzeugt, nicht vom Koordinator, und der Baum
nutzt group tree mit root @teamleiter statt drei Einzelknoten. Das Raster ist group grid mit
columns 2 und vier Formen, dazu drei gestrichelte Linien ohne Pfeil und keine Linie zum
vierten Kreis. @beobachter steht in keinem Knoten und erscheint im Rahmen "Nicht platziert".
Antwortet das Werkzeug mit einem Fehler (etwa width an einer h-Gruppe oder ein Helfer
zusätzlich einzeln platziert), folgt genau ein korrigierter Aufruf, keine Schleife. Abschluss
in unter 8 Minuten.

Warum gut: Prüft Abstammung als Layoutquelle (createdBy über einen Unteragenten), die
Schachtelung (h aus tree und grid), das bewusste Weglassen und den Fehlerpfad mit
korrigierendem zweitem Aufruf.
