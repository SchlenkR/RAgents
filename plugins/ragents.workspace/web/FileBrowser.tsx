...
import { BROWSE_ROOTS, browseChannel, type BrowseListing, type BrowsePreview, type BrowseRoot } from "../contract";
...
const emptyHints: Readonly<Record<BrowseRoot, string>> = {
  workspace: "Im Arbeitsverzeichnis des Laufs liegt noch nichts.",
  files: "In der Dateiablage des Laufs liegt noch nichts.",
};
...
export const fileBrowserPanel = (routePrefix: string) => {
  function FileBrowserPanel({ active, session }: WorkspaceTabContext) {
    const runId = session.session.id;
    const treeMemory = useRef(new Map<string, BrowserMemory>());
    const [root, setRoot] = useState<BrowseRoot>("workspace");
    ...
    useEffect(() => {
      ...
      let alive = true;
      void fetchBrowsePreview(routePrefix, runId, root, selected)
    ...
    useEffect(() => {
      ...
      const load = async (visible: boolean) => {
        ...
        try {
          ...
          const results = await Promise.all(targets.map(async (directory) => ({
            directory,
            listing: await fetchBrowseListing(routePrefix, runId, root, directory).catch((cause: unknown) =>
      ...
      const unsubscribe = eventHub.subscribe({ channel: browseChannel(runId, root), onMessage: () => void load(false) });
      void load(true);
      return () => {
        alive = false;
        unsubscribe();
    ...
    const toggleDirectory = (directory: string) => {
      ...
      setPending(true);
      void fetchBrowseListing(routePrefix, runId, root, directory)
        .then((listing) => {
    ...
    const openFile = (file: string) => {
      ...
      setPreview(undefined);
      setPending(true);
      void fetchBrowsePreview(routePrefix, runId, root, file)
        .then((value) => {
          setPreview(value);
          setError(undefined);
        })
