...
import { Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@aicontainer/web/ui";
...
const isClientInfo = (value: unknown): value is WorkspaceClientInfo => {
  ...
  return typeof raw === "object" && raw !== null
    ...
    && typeof raw.connected === "boolean" && typeof raw.sameMachine === "boolean"
...
const presentationFrom = (presentation: unknown, optionId: string): WorkspaceBindingPresentation => {
  ...
  return { kind: "workspace-binding", clients: raw.clients };
};
...
export const workspaceBindingChoices = (
  clients: readonly WorkspaceClientInfo[],
...
export function WorkspaceBindingControl({ disabled, error, option, setValue }: StartOptionControlContext) {
  ...
  const choices = workspaceBindingChoices(presentation.clients, binding);
...
export function WorkspaceBindingBadge({ option }: StartOptionBadgeContext) {
  ...
  const summary = workspaceBindingSummary(binding);
  return (
