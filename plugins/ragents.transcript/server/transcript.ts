...
import { defineRunFunction, defineToolAvailability, holdsUsable, type JournalEvent, type Orchestration, type RunFunction } from "@aicontainer/ragents";
...
export const createTranscriptTool = (runtime: () => Orchestration): RunFunction =>
  defineRunFunction({
    ...
    run: ({ caller }, _toolCallId, input) => {
      ...
      const handle = input.actor.replace(/^@/, "");
      const actor = view.actors.find((entry) => entry.id === input.actor || entry.handle === handle);
