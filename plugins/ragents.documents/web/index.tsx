...
import type { ToolInfo } from "@aicontainer/web/chat/types";
import {
  DocumentPanel,
  DocumentToolCall,
...
const pathArgumentsFrom = (tool: ToolInfo): { title: string; path: string; format?: string } | undefined => {
  ...
  try {
    ...
    const path = args.path.replace(/^\$\{?RAGENTS_FILES_DIR\}?\//, "");
    return { title: args.title, path, ...(typeof args.format === "string" ? { format: args.format } : {}) };
  } catch {
    return undefined;
  }
};
