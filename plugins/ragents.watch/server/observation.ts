import { actorByReference, type ExecutableActor, type JournalEvent, type JsonValue, type RunView } from "@ragents/engine";
...
export const observeActor = (view: RunView, actorId: string): ActorObservation | undefined => {
  ...
  return {
    ...
    pendingActions: view.actions.filter((action) => action.askedBy === actorId && action.status === "pending").length,
