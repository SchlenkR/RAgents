...
import { DomainError, type ExecutableActor, type JournalEvent, type JsonValue, type Orchestration, type RunView } from "@aicontainer/ragents";
...
import { actorByReference, changesBetween, lastActivityOf, observeActor } from "./observation.js";
...
export class WatchService implements WatchServiceApi {
  ...
  async create(runId: string, callerActorId: string, request: WatchRequest): Promise<WatchSummary> {
    ...
    const source = actorByReference(view, request.source);
    if (!source) throw invalid(`Der beobachtete Actor ${request.source} existiert in diesem Run nicht.`);
    const target = request.target ? actorByReference(view, request.target) : actorByReference(view, callerActorId);
  ...
  async #evaluate(run: RunWatches, watch: WatchRuntime): Promise<void> {
    ...
    const busy = current.actors.find((actor): actor is ExecutableActor => actor.kind !== "human" && actor.id === target.id)?.lifecycle.kind !== "idle"
      ...
      || current.actions.some((action) => action.askedBy === target.id && action.kind === "question" && action.status === "pending");
    if (busy) return;
