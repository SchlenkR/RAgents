...
import { defineRunFunction, type AgentContribution, type RunFunction } from "@aicontainer/ragents";
import { alwaysAvailable } from "@aicontainer/server/plugin-support/tool-availability.js";
...
export const createBrowserFunctions = (browser: RunBrowser): RunFunction[] => [
  defineRunFunction({
    ...
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.open(caller.runId, input.url, signal) }),
  }),
  defineRunFunction({
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }) => ({ ...await browser.snapshot(caller.runId, signal) }),
  }),
  defineRunFunction({
    name: "browser_click",
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.click(caller.runId, input.target, signal) }),
  }),
  defineRunFunction({
    name: "browser_fill",
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.fill(caller.runId, input.target, input.value, signal) }),
  }),
  defineRunFunction({
    name: "browser_select",
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.select(caller.runId, input.target, input.label, signal) }),
  }),
  defineRunFunction({
    name: "browser_press",
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.press(caller.runId, input.target, input.key, signal) }),
  }),
  defineRunFunction({
    name: "browser_check",
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: ({ caller, signal }, _id, input) => browser.check(caller.runId, input, signal),
  }),
  defineRunFunction({
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: async ({ caller, signal }, _id, input) => ({ ...await browser.viewport(caller.runId, input, signal) }),
  }),
  defineRunFunction({
    ...
    available: alwaysAvailable,
    executionMode: "sequential",
    run: ({ caller, signal }, _id, input) => browser.screenshot(caller.runId, input, signal),
  }),
  defineRunFunction({
...
export const createBrowserImageContribution = (browser: RunBrowser): AgentContribution => {
  return {
    ...
    tools: [],
    resolve: ({ runId }) => [{
      name: "ragents.browser.image",
      factory: (agent) => {
        agent.on("tool_result", async (event, context) => {
          if (event.toolName !== "browser_view_screenshot" || event.isError) return;
          try {
            context.signal?.throwIfAborted();
            if (!context.model?.input.includes("image")) throw new Error("Das gewählte Modell unterstützt keine Bilder. Screenshot über die Dokumente ansehen oder ein bildfähiges Modell wählen.");
            const data = (await browser.image(runId)).toString("base64");
            context.signal?.throwIfAborted();
            return { content: [
              { type: "text" as const, text: "Letzte Browseraufnahme dieses Runs." },
