...
export function sheetStatus(messages: readonly Message[], working: boolean, handle: string): SheetStatus {
  ...
  const question = [...messages].reverse().find((message) => message.role === "question" && !message.closed);
  if (question) return { kind: "question", text: `Rückfrage: ${firstLine(question.text) || "Antwort erwartet"}` };
  if (working) {
