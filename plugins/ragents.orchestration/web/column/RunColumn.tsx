...
import { useCenterElements, useColumnHost } from "@aicontainer/web/column/host";
import type { CanvasCenterContext, CanvasElementContribution, CanvasElementDefinition, CardSectionContribution, SessionContext, SessionNavigation } from "@aicontainer/web/PluginRegistry";
...
import { pendingConfirmationFor } from "@aicontainer/plugins/ragents.actor-programs/web/AppsPanel";
...
import { runViewFrom, type RunActor, type RunView } from "../run-view";
...
export function OrchestrationColumn(props: CanvasCenterContext) {
  return <RunColumn key={props.session.session.id} {...props} />;
}

function RunColumn({ canvasElements, cardSections, navigation, renderChat, session, toolbarContainer }: CanvasCenterContext) {
  ...
  const stored = useColumnState(runId);
  ...
  const save = useCallback((patch: Partial<ColumnState>) => saveColumnState(runId, { ...stored, ...patch }), [runId, stored]);
  const elements = useMemo<ColumnElement[]>(() => canvasElements.flatMap(({ Element, select }) => select(session)
  ...
  const actors = useMemo(() => view ? columnActors(view, inspect) : [], [inspect, view]);
  ...
  const layout: ChatLayout = !selectedElement ? "full" : !narrow && stored.chat === "side" ? "side" : "floating";
  ...
  const addressee = view && selectedActor && <AddresseeControl
    ...
    view={view}
  />;
  const layoutToggle = layout !== "full" && toolbarContainer && createPortal(<Button
    aria-label={narrow ? "Chat bleibt unten" : stored.chat === "side" ? "Chat nach unten legen" : "Chat nach rechts legen"}
    className="self-center"
    disabled={narrow}
    onClick={() => save({ chat: stored.chat === "side" ? "bottom" : "side" })}
    size="icon-lg"
    title={narrow ? `Chat bleibt unten, solange die Spalte schmaler als ${settings.sideWidth} Pixel ist` : stored.chat === "side" ? "Chat nach unten legen" : "Chat nach rechts legen"}
    variant="ghost"
  ...
  return <div className="flex min-h-0 min-w-0 flex-1 flex-col" data-column="run" ref={rootRef}>
    ...
    <div className={cn("relative min-h-0 min-w-0 flex-1", layout === "side" ? "grid" : "flex flex-col")} data-chat-layout={layout} style={layout === "side" ? { gridTemplateColumns: `minmax(0,1fr) auto ${chatWidth}px` } : undefined}>
      ...
      <section
        aria-label="Chat"
        className={chatLayoutClass[layout]}
        ...
        onFocusCapture={layout === "floating" ? sheet.open : undefined}
        ...
        ref={sheetRef}
        style={layout === "floating" ? { height: sheet.expanded ? SHEET_HEIGHT : peek } : undefined}
      >
        {layout === "floating" && <button
          aria-expanded={sheet.expanded}
          aria-label={sheet.expanded ? "Chat absenken" : "Chat hochschieben"}
          className="flex h-[18px] w-full flex-none cursor-pointer items-center justify-center [&:hover>span]:bg-foreground focus-visible:outline-2 focus-visible:outline-ring/60 focus-visible:-outline-offset-2"
          onClick={sheet.toggle}
          ref={grabRef}
          title={sheet.expanded ? "Chat absenken" : "Chat hochschieben"}
          type="button"
        ><span aria-hidden className="h-1 w-9 rounded-full bg-border-strong transition-colors" /></button>}
        {status && <button
          className={cn("flex h-6 w-full flex-none cursor-pointer items-center gap-2 px-4 text-left text-[0.7rem] transition-opacity duration-200 in-data-expanded:pointer-events-none in-data-expanded:opacity-0 motion-reduce:transition-none", status.kind === "question" ? "text-warning" : "text-muted-foreground")}
          onClick={sheet.toggle}
          ref={statusRef}
          title={status.text}
          type="button"
...
function useSheet(active: boolean, sheetRef: RefObject<HTMLElement | null>, blocked: boolean, openDelay: number) {
  ...
  const openLater = useCallback(() => { cancel(); timer.current = window.setTimeout(() => setExpanded(true), openDelay); }, [cancel, openDelay]);
  const closeLater = useCallback((delay: number) => {
    cancel();
    timer.current = window.setTimeout(() => {
      if (guard.current.held || guard.current.blocked || sheetRef.current?.contains(document.activeElement)) return;
      setExpanded(false);
    }, delay);
  }, [cancel, sheetRef]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape" && !guard.current.blocked) close(); };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [active, close, expanded]);
  return { expanded, open, openLater, closeLater, close, toggle };
}
...
function useSheetPeek(active: boolean, sheetRef: RefObject<HTMLElement | null>, grabRef: RefObject<HTMLButtonElement | null>, statusRef: RefObject<HTMLButtonElement | null>, actorId: string | undefined) {
  ...
  useLayoutEffect(() => {
    ...
    const composer = sheetRef.current?.querySelector<HTMLElement>('[data-chat="composer"]');
    if (!grab || !status || !composer) throw new Error("Das Sheet der Spalte findet Griff, Statuszeile oder Eingabe nicht.");
    const measure = () => setPeek(grab.offsetHeight + status.offsetHeight + composer.offsetHeight);
...
function ElementChip({ centered, definition, onSelect, selected, session, view }: {
  ...
  const app = programs?.listing?.apps.find((candidate) => candidate.id === definition.id);
  const title = definition.title ?? definition.id;
  const attention = elementNeedsAttention(view, definition, app !== undefined && pendingConfirmationFor(session, app) !== undefined);
...
function ColumnStage({ centered, element, navigation, peek, runId, session }: {
  centered: boolean;
  ...
  return <section aria-label={`Mini-App ${title}`} className="group/stage relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-card/55" style={peek > 0 ? { paddingBottom: peek } : undefined}>
    ...
    <div className="relative flex min-h-0 flex-1 flex-col" data-column="stage">
      {centered
        ? <div className="m-auto grid gap-3 p-4 text-center text-[0.76rem] text-muted-foreground" role="status">
          ...
          <Button onClick={() => host.returnToColumn(runId, definition.id)} size="sm" variant="outline">Zurück in die Spalte</Button>
        </div>
        : <Element definition={definition} navigation={navigation} presentation="tiled" session={session} />}
    </div>
  </section>;
}
...
function ActorColumnChat({ actor, cardSections, navigation, onNavigate, session, toolbarLeft, view }: {
  actor: RunActor;
  ...
  return <div className="flex min-h-0 min-w-0 flex-1 flex-col">
    ...
    <ChatPanel className="flex-1" composer={<ActorChatControls actor={actor} presentation="column" running={running} surface="column" toolbarLeft={toolbarLeft} view={view} />}>
      <ActorChat actor={actor} conversation={session.actorConversations?.[actor.id]} historyError={session.conversationError} onNavigate={onNavigate}
        presentation="inspector" primaryMessages={session.messages} running={running} surface="column" view={view} />
    </ChatPanel>
  </div>;
}
