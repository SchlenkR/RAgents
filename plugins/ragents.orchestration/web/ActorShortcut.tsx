...
import { useCanvasController, type SessionContext } from "@aicontainer/web/PluginRegistry";
...
import { useCanvasPresentation } from "./tiled-view-settings";
...
export function ActorShortcut({ actor, view, session, open, onToggle, onClose, visible }: {
  ...
  const tiled = useCanvasPresentation(session.session.id, view).presentation.mode === "tiled";
  ...
  return <div className="flex items-stretch [&[hidden]]:hidden" hidden={!visible}>
    <ToolbarItem as="button" aria-controls={visited ? panelId : undefined} aria-expanded={open} aria-haspopup="dialog"
      ...
      draggable={tiled && technical} onDragStart={(event) => {
        if (!tiled || !technical) { event.preventDefault(); return; }
        event.dataTransfer.setData(CANVAS_TILE_DRAG_TYPE, `@${actor.handle}`);
        event.dataTransfer.effectAllowed = "move";
        close();
      }}
      ref={buttonRef} title={`${type}: ${label} öffnen${tiled && technical ? ". Zum Andocken auf eine Kachel ziehen." : ""}`} type="button">
