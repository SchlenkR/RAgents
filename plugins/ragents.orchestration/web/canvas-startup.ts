import type { ChatStartupStatus } from "@aicontainer/server/chat-events";
import { isPendingRunActorInput, type RunView } from "./run-view";

export interface CanvasStartupState {
...
export function canvasStartupState({ view, startup, connected, running, error }: {
  ...
  if (view?.actions.some((action) => action.status === "pending")) return {
    kind: "waiting", title: "Deine Antwort wird benötigt", detail: "Im Chat wartet eine Rückfrage auf Dich.",
  };
