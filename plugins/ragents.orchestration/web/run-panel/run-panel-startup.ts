...
export function useRunPanelStartup(state: CanvasStartupState | undefined, connected: boolean, content: boolean): CanvasStartupState | undefined {
  const [contentSeen, setContentSeen] = useState(content);
  const [held, setHeld] = useState<CanvasStartupState>();
