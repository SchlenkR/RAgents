...
import type { CanvasElementContribution, CardSectionContribution, EntityReference, SessionContext, SessionNavigation } from "@aicontainer/web/PluginRegistry";
...
import { actorSurface, type RunView } from "./run-view";
  ...
  const items: CanvasTileItem[] = [
    ...actors.map((actor): CanvasTileItem => ({
      ...
      content: actor.kind === "agent"
        ? <ActorTile actor={actor} view={view} cardSections={cardSections} session={session} navigation={navigation} onSelect={onSelect} />
    ...
    ...canvasElements.flatMap(({ Element, select }) => select(session)
      ...
      .map((definition): CanvasTileItem => ({
        ...
        surface: "app",
        content: <Element definition={definition} navigation={navigation} session={session} presentation="tiled" />,
      }))),
  ];
