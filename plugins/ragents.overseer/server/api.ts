...
import { DomainError, implement, type JsonValue, type MethodContribution, type PluginHost } from "@aicontainer/ragents";
...
const resolve = async (context: ManagementContext, reference: string) => {
...
const create = async (context: ManagementContext, body: CreateInput) => {
...
export function managementMethods(context: ManagementContext): MethodContribution[] {
  return [
    implement(overseerContracts.listRuns, async () =>
      (await context.directory.describe(await context.management().list())).map(({ id, ...entry }) => ({ runId: id, ...entry }))),
    implement(overseerContracts.createRun, (input) => create(context, input)),
    implement(overseerContracts.readRun, async ({ run }) => context.management().view((await resolve(context, run)).id)),
    implement(overseerContracts.readEvents, async ({ run, after, limit, type }) => {
      const found = await resolve(context, run);
    ...
    implement(overseerContracts.sendMessage, async ({ run, message }) => {
      const found = await resolve(context, run);
      await context.management().send(found.id, message.trim());
      return { ...identity(found), accepted: true as const };
    }),
    implement(overseerContracts.stopRun, async ({ run }) => {
      const found = await resolve(context, run);
      await context.management().stop(found.id);
      return { ...identity(found), stopped: true as const };
    }),
    implement(overseerContracts.readCatalog, () => ({
