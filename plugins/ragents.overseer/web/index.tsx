...
import { Button, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Popover, PopoverContent } from "@aicontainer/web/ui";
import { RunModalContext } from "@aicontainer/web/ui/dialog";
import { XIcon } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { ChatMessages } from "@aicontainer/web/chat/ChatMessages";
import { ChatInputToolbar, type ChatInputHandle } from "@aicontainer/web/chat/ChatInputToolbar";
import { DetailModeSwitch } from "@aicontainer/web/chat/DetailModeSwitch";
import { useChat } from "@aicontainer/web/chat/useChat";
  ...
  return <div className={toolbarClass} data-open={open} data-slot="overseer-toolbar" ref={anchor}
    ...
    <div className="min-w-0 flex-1" onClick={(event) => { if (event.target instanceof HTMLTextAreaElement) requestOpen(); }}>
      <ChatInputToolbar {...attachments} detailsContainer={detailsContainer} layout="toolbar" handleRef={composer}
        ...
        onStop={writable ? () => { if (!resetPending.current) perform(chat.stop); } : undefined}
