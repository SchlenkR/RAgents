...
const processesPlugin: RAgentsPlugin = {
  ...
  register: (host) => {
    ...
    host.methods(...createProcessMethods({ observer, ensureSession, terminate: (runId, processId, context) => terminator.stop(runId, processId, context) }));
    host.channels(createProcessChannel({ observer, ensureSession }));
    host.lifecycle({
