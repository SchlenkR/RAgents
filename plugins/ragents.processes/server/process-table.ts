...
import { RUN_MARKER_ENV, runMarkerOf } from "@aicontainer/server/plugin-support/run-marker.js";
import type { RunProcessPort } from "../contract.js";
...
export interface ProcessTable {
  ...
  listeningPorts: (pids: readonly number[]) => Promise<Map<number, RunProcessPort[]>>;
}
...
const addPort = (ports: RunProcessPort[], port: RunProcessPort): void => {
...
const portFromLsofName = (name: string): RunProcessPort | undefined => {
...
export const parseLsofListeners = (text: string): Map<number, RunProcessPort[]> => {
  const listeners = new Map<number, RunProcessPort[]>();
  let current: RunProcessPort[] | undefined;
  for (const line of text.split("\n")) {
...
const readLinuxRecord = async (pid: number): Promise<ProcessRecord | undefined> => {
  let stat: string;
  let status: string;
  let cmdline: Buffer;
  try {
...
const readLinuxMarker = async (pid: number): Promise<string | undefined> => {
  let environ: Buffer;
  try {
...
const linuxListeningInodes = async (): Promise<Map<string, RunProcessPort>> => {
  const inodes = new Map<string, RunProcessPort>();
...
const linuxSocketInodes = async (pid: number): Promise<string[]> => {
...
const linuxProcessTable = (): ProcessTable => ({
  list: async () => {
    const records = await Promise.all((await numericEntries(PROC)).map(readLinuxRecord));
    return records.filter((record): record is ProcessRecord => record !== undefined);
  },
  runMarkers: async (pids) => {
    const markers = new Map<number, string>();
    for (const pid of pids) {
      const marker = await readLinuxMarker(pid);
      if (marker) markers.set(pid, marker);
    }
    return markers;
  },
  listeningPorts: async (pids) => {
    ...
    const inodes = await linuxListeningInodes();
    if (inodes.size === 0) return listeners;
    for (const pid of pids) {
      const ports: RunProcessPort[] = [];
