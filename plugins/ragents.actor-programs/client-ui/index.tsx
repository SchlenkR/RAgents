...
import { ChatPanel } from "../../../apps/web/src/chat/ChatPanel";
...
const appChat = (): ChatConnection => {
  ...
  if (!bridge) throw new Error("Die Host-Bridge der Actor-Ansicht fehlt.");
  return bridge.chat;
};
...
function ActorChat({ actor, ...props }: ActorChatProps) {
  ...
  return <ChatView {...props}
    ...
    emptyState={props.emptyState ?? (snapshot === undefined ? <p className="px-4 py-3 text-[13px] text-muted-foreground">Gespräch wird geladen ...</p> : undefined)}
