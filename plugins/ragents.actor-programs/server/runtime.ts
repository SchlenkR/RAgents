...
import { agentTools, assertJsonValue, canonicalHash, defineRunFunction, defineToolAvailability, emptyUsage, runCapabilityContractHash, scriptInputOf, schemaComplaints, type Actor, type ActorProgramExecutor, type RunFunction, type CommandContext, type ExecutableActor, type JsonValue, type Orchestration, type PluginContext, type RunCapabilityDescriptor, type RunView, type TurnRequest, type TurnResult, } from "@aicontainer/ragents";
import { ACTOR_PROGRAMS_STATE_ID, ACTOR_INVOCATIONS_STATE_ID, ACTOR_SCRIPT_STATE_ID, ACTOR_STATE_ID, resolveActorView, type ActorDataState, type ActorFunctionDefinition, type ActorFunctionInvocation, type ActorProgramDefinition, type ActorProgramState, type ActorScriptState, type ActorViewListing, } from "../contract.js";
import type { ActorProgramSource, ActorProgramsService } from "@aicontainer/server/plugin-support/actor-programs/service.js";
import { jsonValue, jsonBytes, MAX_INVOCATIONS_STATE_BYTES } from "@aicontainer/server/plugin-support/actor-programs/limits.js";
import { agentCapabilityBinding, operatorCapabilityBinding } from "@aicontainer/server/plugin-support/actor-programs/capability-resolver.js";
import type { ActorOperationPort } from "@aicontainer/server/plugin-support/actor-programs/operations.js";
import { compileClientProject, installClientSdk } from "./client-compiler.js";
import { compileAppBackend, installServerSdk, prepareAppProject, prepareAppWorkspace, projectSourceFiles, readAppPackage, typecheckServerProject } from "./app-project.js";
import { syncWorkspaceOwnership } from "@aicontainer/server/plugin-support/workspace-ownership.js";
import { runManagedProcess } from "@aicontainer/server/plugin-support/managed-process.js";
import type { SandboxProcessContext } from "@aicontainer/server/plugin-support/workspace-sandbox-host.js";
import { runModuleTemplates, templateById } from "./templates.js";
import { FRAME_BODY_CLASS, FRAME_DOCUMENT_CLASS, FRAME_ROOT_CLASS } from "../client-ui/frame-layout.js";
import { buildTailwind } from "./tailwind.js";
import type { AskService } from "@aicontainer/plugins/ragents.ask/server/contract.js";
...
export class ActorProgramRuntime implements ActorProgramsService, ActorProgramExecutor {
    ...
    resolveActor(runId: string, callerId: string, reference: string): ExecutableActor {
        ...
        const found = this.view(runId).actors.find((candidate) => reference === "self" ? candidate.id === callerId : candidate.handle === name);
        if (!found)
    ...
    async check(runId: string, name: string, callerId: string, signal?: AbortSignal, reference?: string): Promise<CompiledProgram> {
        ...
        const contract: import("./app-project.js").AppContract = backend?.contract ?? { state: objectSchema, functions: {} };
        ...
        for (const view of pkg.views ?? []) {
            ...
            if (!client.valid)
                throw new Error(client.diagnostics.map((item) => `${item.fileName} ${item.message}`).join("\n"));
    ...
    async #activate(context: CommandContext, runId: string, name: string, idle: boolean, signal?: AbortSignal, reference?: string) {
        ...
        try {
            ...
            if (tests.length) {
                const processContext = await this.#options.processContextFor(runId);
                let output = "";
            ...
            await syncWorkspaceOwnership(definition.directory, await this.#options.processContextFor(runId));
    ...
    startFunctionInvocation(runId: string, actorHandle: string, revision: string, functionId: string, requestId: string, input: unknown, permit = this.invocationPermit(runId)) {
        const program = this.programs(runId).find((entry) => entry.actorHandle === actorHandle.replace(/^@/, ""));
        if (!program)
