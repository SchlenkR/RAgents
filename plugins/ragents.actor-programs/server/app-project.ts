...
import { canonicalHash, typeScriptTypesFromSchema, type NativeTypeScriptExecutor, type RunCapabilityDescriptor, scriptStdDeclarations } from "@aicontainer/ragents";

const webRequire = createRequire(new URL("../../../apps/web/package.json", import.meta.url));
const rootRequire = createRequire(new URL("../../../package.json", import.meta.url));
const serverRequire = createRequire(new URL("../../../apps/server/package.json", import.meta.url));
const serverTypeRoot = fileURLToPath(new URL("../../../apps/server/node_modules/@types", import.meta.url));
const webLibraries = ["react", "react-dom", "@types/react", "@types/react-dom", "@base-ui/react", "class-variance-authority", "cn", "lucide-react"];
const libraries = [...webLibraries, "@types/node", "typescript", "tsx", "typebox", "esbuild"];
const libraryDirectory = (name: string): string => {
  const resolver = webLibraries.includes(name) ? webRequire
...
export const prepareAppDependencies = async (directory: string): Promise<void> => {
  ...
  for (const name of libraries) await link(libraryDirectory(name), path.join(directory, "node_modules", name));
...
export const prepareAppWorkspace = async (directory: string): Promise<string> => {
  ...
  const dependencies = Object.fromEntries(libraries.map((name) => [name, `link:${libraryDirectory(name)}`]));
