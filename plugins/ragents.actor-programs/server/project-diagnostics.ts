...
import type { AgentContribution } from "@aicontainer/ragents";
import { projectSourceFiles } from "./app-project.js";
...
export const createProjectDiagnostics = (options: ProjectDiagnosticsOptions) => {
  ...
  const contribution: AgentContribution = {
    id: entryType,
    tools: [],
    resolve: ({ runId, agentId }) => options.applies?.(runId, agentId) === false ? [] : [{
      name: entryType,
      factory: (agent) => {
        agent.on("context", async (event, context) => {
          context.signal?.throwIfAborted();
          const stored = context.sessionManager.getBranch().filter((entry) => entry.type === "custom" && entry.customType === entryType).at(-1);
          const previous = stored?.type === "custom" && isSnapshot(stored.data) ? stored.data : { version: 1 as const, programs: {} };
          ...
          try {
            ...
            for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
              ...
              changed = true;
              if (errors.length === 0) {
                try { await options.check(runId, entry.name, agentId, context.signal); }
                catch (error) {
                  context.signal?.throwIfAborted();
                  errors = errorLines(error);
                }
          ...
          if (!changed) return;
          agent.appendEntry(entryType, next);
          ...
          return { messages: [...event.messages, { role: "custom" as const, customType: entryType, content, display: false, timestamp: Date.now() }] };
        });
      },
    }],
  };
