...
import type { ChatSnapshot } from "../client-ui/contracts";
import { actorChatMessages } from "@aicontainer/plugins/ragents.orchestration/web/actor-conversation";
import { runViewFrom } from "@aicontainer/plugins/ragents.orchestration/web/run-view";
...
export const resolveChatActor = (session: SessionContext, target: string) => {
  ...
  const actor = target === "primary"
    ...
    : view.actors.find((entry) => entry.handle === target.replace(/^@/, ""));
