...
import { Alert, Badge, Button, cn, Empty, EmptyContent, EmptyDescription, EmptyHeader, EmptyMedia, EmptyTitle, Spinner } from "@aicontainer/web/ui";
...
import { ACTOR_PROGRAMS_STATE_ID } from "../contract";
import type {
  SessionContext,
...
export function ActorProgramsProvider({
  ...
  return <ActorProgramsContext.Provider value={value}>{children}</ActorProgramsContext.Provider>;
}
...
export const pendingConfirmationFor = (
  ...
  return view.actions.find((action) => action.kind === "question"
...
export function HostConfirmation({ action, session }: { action: RunAction; session: SessionContext }) {
  ...
  const answer = async (text: string) => {
    ...
    setError(undefined);
    try {
      await session.respond(action.id, text);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
      setBusy(false);
    }
  ...
  return (
    <section aria-label="Host-Bestätigung" className="grid flex-none gap-2 border-b border-border-soft bg-warning-soft px-workspace-inset py-2.5" data-slot="host-confirmation">
      ...
      {busy
        ...
        : (
          <QuestionCard
            onAnswer={access.can("runs.write") ? (text) => void answer(text) : undefined}
            question={{
              callId: action.id,
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>Das ausgewählte Actor-Funktion ist in diesem Lauf nicht mehr installiert.</EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>Von Agenten erstellte Actor-Funktionen dieses Laufs erscheinen hier.</EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  }
