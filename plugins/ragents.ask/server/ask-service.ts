...
const answerOf = (decision: "approved" | "dismissed", response: string | null): string =>
  decision === "approved" ? response ?? "" : DISMISSED_ANSWER;

const resolvedAnswerOf = (action: Action): string | null =>
  action.status === "pending" ? null : answerOf(action.status, action.response);
...
export class RuntimeAskService implements AskService {
  ...
  ask(call: AskCall, request: AskRequest, signal: AbortSignal | undefined): Promise<string> {
    ...
    runtime.proposeAction(context, call.runId, {
      kind: "question",
      question: { options: request.options, multi: request.multi === true },
  ...
  answer(runId: string, actionId: string, input: { answer?: string; dismiss?: boolean }): void {
    ...
    runtime.resolveAction(
      ...
      actionId,
      input.dismiss === true
        ? { decision: "dismissed", response: null }
        : { decision: "approved", response: input.answer ?? null },
    );
  }
  ...
  #onJournal(events: JournalEvent[]): void {
    for (const event of events) {
      ...
      if (resolved?.kind !== "question") continue;
      const answer = answerOf(event.payload.decision, event.payload.response);
